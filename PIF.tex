...
the \textit{information set} used by the forecaster. Therefore the forecasters operate under the same probability model but make predictions based on different information sets. 
...
\begin{proposition}
\label{covstr}
Suppose $\E[Y|X_j]  = X_j$ for all $j =1, \dots, N$. Let $\F_j = \sigma(X_j)$ and consider a sub-$\sigma$-field $\sigma(X_i) = \F_i \subseteq \F_j$ for some other forecast $X_i$. Then,
\begin{enumerate}[i)] 
...
\end{enumerate}
\end{proposition}
This proposition links the observed forecasts and the unknown $Y$. In particular, item \ref{first}) shows that the mean of $Y$ is simply the mean of the forecasts. Furthermore, if the forecasters report probability estimates, the prior distribution can be estimated from the data. Item \ref{second}) shows that $\Cov(X_j, Y) = \Var(X_j)$ because $Y = \E[Y | \F]$ and $\F_j \subset \F$ for all $j = 1, \dots, N$. This explains how the covariance matrix of the observables, namely the $X_j$'s extends to the unknown $Y$. Such extension allows $Y$ to be regressed on the $X_j$'s without a separate training set. The regression is performed by the \textit{revealed} aggregator $X'' := \E [Y \|
\F'']$, where $\F''$ is the $\sigma$-field generated (or revealed) by the forecasts
$\{X_j : j = 1, \dots, N \}$. This aggregator optimally utilizes the information in the forecasts and is therefore the relevant estimator in each specific partial information model. Finally, item \ref{third}) shows that the variance of the forecasts increases in the amount of information. 
...
\label{gaussian}
To apply the partial information framework in practice requires a joint probability distribution for the model variables. Any distribution can be used as long as it can incorporate the covariance structure described in the previous section.  In many applications, however, $Y$ and the $X_j$'s may not share a common support, making it difficult to directly state a joint distribution for them. One solution is to use a set of hidden variables, called the \textit{information variables}, that follow a joint multivariate Gaussian distribution with the correct covariance structure. More specifically, denote the information variables with $Z_0, Z_1, \dots, Z_N$ and let
\begin{align}
...
\end{align}
This \textit{distribution of information} has an easy interpretation: $\delta_j := \Var(Z_j)$ is the amount of information used by Forecaster $j$ and $\rho_{ij} := \Cov (Z_{i} , Z_{j})$ is the information overlap between Forecasters $i$ and
$j$. Therefore the sub-matrix $\bSigma_{22}$ is called the \textit{information structure}. The information variables link to $Y$ and the $X_j$'s via a \textit{link function} and the model assumptions. First, if $\mathcal{Y}$ denotes the support of $Y$, then 
 $Y = g(Z_0)$, where $g: \R \to \mathcal{Y}$ is some surjective \textit{link function} specific to the model instance. Second, given that the forecasts are assumed conditionally unbiased, Forecaster $j$ predicts $X_j = \E[Y | Z_j]$. This gives a very flexible specification, yielding a broad range  of partial information models. Examples are discussed in Section \ref{empirical} that illustrates the model on several different types of outcomes.  
...
The choice of the Gaussian distribution can be motivated both from the intuitive and technical points of view. First, \citet{broomell2009experts} discuss a model under which
...
$H_1 , \ldots , H_r$ influencing the quantity of interest.  Proposing a linear model for subjective
...
to model the forecasters' observations as jointly Gaussian.  Second, both the multivariate and conditional Gaussian distributions are well-studied and have simple forms. This is particularly useful when deriving the revealed aggregator.   In particular, if $\Z = (Z_{1}, \dots, Z_{N})'$, then $$Z_0 | \Z \sim \mathcal{N}\left(\bSigma_{12} \bSigma_{22}^{-1} \Z, 1-\bSigma_{12} \bSigma_{22}^{-1}\bSigma_{21}\right).$$ This leads to a closed-form revealed aggregator $X''$ in many different applications of the model. For more motivation and interpretation, see \cite{satopaamodeling}.
...
Estimating the information structure $\bSigma_{22}$ is equivalent to estimating a covariance matrix under the partial-information-motivated pattern described in (\ref{NExperts}). More specifically, if $\mathcal{S}_{+}^N$ is the set of  symmetric positive semidefinite $N \times N$ matrices and 
...
\diag(\M) &\M\\
 \end{matrix}\right),
\end{align*}
where $\M \in \mathcal{S}^{N}_+$ and $\diag(\M)$ is a vector of the diagonal elements of $\M$, then estimation must performed under the condition $h(\bSigma_{22}) \in \mathcal{S}_{+}^{N+1}$. 
...
The first step is to define $\bSigma := h(\bSigma_{22})$ and project $h(\boldsymbol{S}_Z)$ instead of $\boldsymbol{S}_Z$. 
