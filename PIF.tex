...
% 
As was mentioned earlier, the Good Judgment Project has recruited thousands of forecasters to make probability predictions on different geopolitical events. This subsection analyzes the predictions made during the second year of the project.  
%Since 2011 t%Before the forecasts can be aggregated, however, several practical matters must be discussed. First, 
Each event is associated with a timeframe during which the forecasters make and update their predictions. To illustrate, two of
the questions were: ``Will Moody's issue a new downgrade on the long-term ratings for any of the eight major French banks between 30 July 2012 and 31 December 2012?'' and ``Will the Palestinian group Islamic Jihad significantly violate its cease-fire with Israel before 30 September 2012?''. The predictions made for these two events are shown in Figure \ref{Examples}. Each prediction problem closes and no more forecasts are allowed once the corresponding event resolves. Overall, the events have very different timeframes, ranging from several weeks to more than a year. For instance, the events in Figures \ref{Example1} and \ref{Example2} have timeframes of 154 and 52 days, respectively. Uncertainty in the event is typically the highest at the beginning of the timeframe and then vanishes by the resolution date. Given that this paper does not focus on dynamic data, only forecasts made within some interval of time should be considered for each event. The goal is to choose an interval during which predicting the correct outcome is neither trivial nor extremely difficult because in such cases competing aggregators are unlikely to present meaningful differences. Therefore, the interval used in this paper is $30$ to $100$ days before the resolution date. This is illustrated with the shaded regions in Figure \ref{Examples}. Some events had timeframes less than $30$ days and were omitted. The interval choice, however, was a compromise that maintained sufficient uncertainty in the events without discarding too much data. The final dataset has $48$ binary events. 

\marginpar{mention: also justify the cut-off a bit more}
...
\centering
        ...
        \begin{subfigure}{0.49\textwidth}
                \includegraphics[width=\textwidth]{1127_63}
        ...
        \end{subfigure}
        \begin{subfigure}{0.49\textwidth}
                \includegraphics[width=\textwidth]{1160_75}
        ...
        \caption{Two events and the associated probability forecasts over time. Neither of these events occurred. Therefore the forecasts converge towards the correct answer as the resolution day (the right most point) approaches. Only forecasts made within the shaded region are used in the analysis.}
        \label{Examples}
\end{figure}


Recall that the partial information framework is designed for noise-reduction and hence assumes forecasts to be conditional unbiasedness. Unfortunately, the forecasters of the GJP are not conditionally unbiased, i.e., \textit{calibrated}, as this condition is more often called in the probability forecasting literature. In fact, human experts are generally known to lack calibration.  Even though the calibration of the forecasts can be improved via pre-processing mechanisms, such as Platt scaling \citep{platt1999probabilistic} or Isotonic regression \citep{zadrozny2001obtaining}, these techniques require a training set with known outcomes and are hence not in the scope of this paper. Consequently, the GJP forecasts are not pre-processed. The results, however, suggest that the Gaussian model remains robust and is able out-perform competing aggregators even when the forecasts are not extremely well-calibrated. 
...
All the events considered in this section have two possible outcomes. Therefore $Y_k \in \{0,1\}$ for all $k = 1, \dots, K$. These outcomes are linked to the Gaussian information pool via the following function:
\begin{align*}
...
All the unknown parameters of this model can be estimated. This begins with an unconstrained estimate of $\bSigma_{22}$. If the $t_k$'s do not change much, a reasonable and simple estimate can be obtained  by transforming the sample covariance matrix  $\SS_P$ of the probit scores $P_{jk} := \Phi^{-1}(X_{jk})$; that is,  if $\dd := \diag(\SS_P) / (1+\diag(\SS_P))$, where the division is element-wise, then an unconstrained estimator of $\bSigma_{22}$ is given by
\begin{align*}
\SS_Z &= \Diag(1-\dd)^{1/2} \SS_P \Diag(1-\dd)^{1/2}
\end{align*}
...
% observing that $P_{jk} := -\Phi^{-1}(X_{jk})\sqrt{1-\delta_j} \sim \mathcal{N}(t_k, \delta_j)$. 
 The threshold $t_k$ is then estimated by the precision-weighted average of these quantities: 
\begin{align*}
\hat{t}_k &= -\frac{\sum_{j=1}^N \sqrt{1-\hat{\delta}_j}P_{jk}/\hat{\delta}_j}{\sum_{j=1}^N 1/\hat{\delta}_j},
\end{align*}
where $\hat{\delta}_j$ represent the current projected estimates of $\delta_j$. This procedure is repeated over a grid of $100$ candidate values $\nu$ between $10$ and $1,000$. The final value is chosen with the conditional validation technique discussed in Section \ref{condSelection}.


This subsection benchmarks the revealed aggregator (\ref{RevAgg}) against the common averaging aggregators: average probability, median probability, average probit-score, and average log-odds.  To avoid infinite log-odds
