...
% For each event The  aggregators then observe $\Z_k^{-}$, where $\Z_k^{-} = (Z_{B_{1},k}, \dots, Z_{B_N, k})'$ holds only the forecasters' information variables such that $\Z_k = (Z_{S,k}, {\Z_k^{-}}')'$, 
 and predict $Y_{k}$ based on this information. The process is repeated a $5,000$ times under different values of $K$ and $N$, ranging from $5$ to $35$ with constant increments of $5$. 
...
\vspace{-3.5em}
    ...
    \begin{subfigure}{0.5\textwidth}
                ...
                \subcaption{Predictions errors under different value of $K$ and fixed $N = 20$.}
                \label{Nfixed}
        \end{subfigure}%
        \begin{subfigure}{0.5\textwidth}
                \includegraphics[width=\textwidth]{simResXN.pdf}
                \subcaption{Predictions errors under different value of $N$ and fixed $K = 20$.}
                                \label{Kfixed}
        \end{subfigure}
