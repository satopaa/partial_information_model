...
\begin{abstract}
Prediction polling is an increasingly popular form of crowdsourcing that inquires its participants about their best estimates of some quantity of interest, such as tomorrow's stock price or the likelihood of a particular future event happening. Historically, randomness in scientific estimation has  been generally
assumed to arise from unmeasured or uncontrolled factors. However, 
when combining subjective estimates, heterogeneity
stemming from differences in the participants' information is often
more important than measurement noise. This paper formalizes information diversity as an alternative source of heterogeneity and introduces a novel modeling framework that is particularly suited for prediction polls. A practical specification of this framework is proposed and applied to the task of aggregating probability and point estimates from two real-world prediction polls. In both cases our model outperforms the measurement-error-based aggregators, hence providing evidence in favor of information diversity being the more important source of heterogeneity. 
...
The current paper introduces statistical methodology designed specifically for the rapidly growing practice of prediction polling. The methods are illustrated on two of the most common types of responses, namely prediction and point forecasts. In both cases the data arise from real-world prediction polls.  More specifically, the probability forecasts are from a tournament initiated in 2011 by the Intelligence Advanced Research Projects Activity (IARPA) as means to acquire accurate predictions on future international political events. Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
...
Over a period of time these forecasters were then asked to estimate probabilities of the events specified by IARPA. For each event the forecasters were allowed to make and update their estimates until the event resolved. To illustrate, Figure \ref{Example_prob} shows the forecasts for one of these events. In this example forecasting began on 30 July 2012 and ended on 30 December 2012 when the event resolved as "No" (represented by the red line at $0.0$). 
...
%        \begin{subfigure}[t]{0.495\textwidth}
                ...
                \caption{Point forecasts of the weights of $20$ different people. The boxplots have been sorted to increase in the true weight. Some extreme values were omitted for the sake of clarity.}
                                \label{Example_point}
%        \end{subfigure}             
...
\marginpar{This really is a modeling framework; instead of taking the mean, we can built different kinds of models depending on the context}
\marginpar{The framework is introduced by starting very general then going more specific.}
...
function $L(X_i, Y)$. Many choices are possible, and
...
\marginpar{IN general unsupervised aggregation leads to garbage in garbage out methods.}
...
the \textit{information set} used by the forecaster. This means that Forecaster $j$ may have access to some larger $\sigma$-field $\G$ but only decides to use $\F_j \subseteq \G$. For the purposes of aggregation, any available information discarded by the forecaster may as well not exist. The forecasters then operate under the same probability model but make predictions based on different information sets. 
...
% $X_j = \E [Y | \F_j]$ only requires the existence of a probability model, together with the assumption of conditional unbiasedness. 
 ...
 \marginpar{must explain that this is information used. This is important for the second app; in fact must be more careful about this throughout the paper. This is important distinction}
...
\begin{align}
\Cov((Y, X_1, \dots, X_N)) &=  \left( \begin{matrix} 
 \Var(Y)  & \diag(\bSigma_X)'  \\
\diag(\bSigma_X) & \bSigma_X \\
\end{matrix} \right), \label{cov_str}
\end{align}
where $\diag(\bSigma_X)$ is a vector of the diagonal elements of $\bSigma_X$
%
...
%  This explains how the covariance matrix of the observables, namely the $X_j$'s extends to the unknown $Y$ (see Equation (\ref{NExperts})).
   Such an extension allows $Y$ to be regressed on the $X_j$'s without a separate training set of past prediction problems with known outcomes. The resulting estimator, called the \textit{revealed} aggregator, is given by $X'' := \E [Y \|
...
\end{align}
This \textit{distribution of information} has an easy interpretation: $\Var(Z_0) = 1.0$ is the total amount of information, $\delta_j := \Var(Z_j)$ is the amount of information used by Forecaster $j$, and $\rho_{ij} := \Cov (Z_{i} , Z_{j})$ is the information overlap between Forecasters $i$ and
$j$. Therefore the sub-matrix $\bSigma_{22}$ is naturally called the \textit{information structure}. The information variables link to $Y$ and the $X_j$'s via a \textit{link function} and the model assumptions. First, if $\mathcal{Y}$ denotes the support of $Y$, then 
 $Y = g(Z_0)$, where $g: \R \to \mathcal{Y}$ is some surjective link function specific to the model instance. For instance, if the prior distribution of $Y$ is $\mathcal{N}(\mu_0, \sigma^2_0)$, then $\mathcal{Y} = \R$ and $g(Z_0) = Z_0 \sigma_0 + \mu_0$. Therefore  the link function is assumed to transform $Z_0$ such that $Y$ follows the correct prior distribution. Second, given that the forecasts are conditionally unbiased, Forecaster $j$ predicts $X_j = \E[Y | Z_j]$. This is overall  a very flexible specification that yields a broad range  of different partial information models. Examples are discussed in Section \ref{empirical} that illustrates the \textit{Gaussian model} on several different types of outcomes.  
...
to model the forecasters' observations as jointly Gaussian.  Second, both the multivariate and conditional Gaussian distributions are well-studied and have simple forms. This is especially useful in deriving the revealed aggregator.   In particular, if $\Z = (Z_{1}, \dots, Z_{N})'$, then $$Z_0 | \Z \sim \mathcal{N}\left(\diag(\bSigma_{22})' \bSigma_{22}^{-1} \Z, 1-\diag(\bSigma_{22})' \bSigma_{22}^{-1}\diag(\bSigma_{22})\right).$$ This leads to a closed-form revealed aggregator $X''$ in many different applications of the model. The term $\diag(\bSigma_{22})' \bSigma_{22}^{-1}\diag(\bSigma_{22})$ can be interpreted as the total amount of information revealed by the $N$ forecasters. The conditional mean $\diag(\bSigma_{22})' \bSigma_{22}^{-1} \Z$, on the other hand, is the single best estimate of $Z_0$ given $\Z$. This term does not depend on $\Var(Z_0)$; that is, the conditional mean is completely defined by how much the forecasters know relative to each other -- not by how much they know in absolute terms. 
...
For more motivation and interpretation, see \cite{satopaamodeling}.
