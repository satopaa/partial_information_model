...
% This section, however, focuses on predicting global events that are of particular interest to the Intelligence Advanced Research Projects Activity (IARPA).
 ...
 In 2011 the Intelligence Advanced Research Projects Activity (IARPA) launched a forecasting tournament as a means to acquire accurate predictions on global events such as ``Will Serbia be officially granted EU candidacy by 31 December 2011?'' and ``Will Zimbabwe commence a presidential election before 1 January 2013?". Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
...
Once the predictions have been collected, the challenge is to combine them into a single consensus. This is typically done for the sake of decision-making and improved accuracy \citep{armstrong2}. Principled aggregation, however, requires an assumption about the source of heterogeneity among the forecasts. In particular, it is necessary to understand how the forecasts differ from the true value of the target quantity. For the past several decades, potentially due to the early forms of data collection, measurement error has served as the main source of heterogeneity. This approach has become the standard and is often applied in practice even when data variation is dominated  by causes besides error in measurement. For instance, the measurement error may be reasonable in modeling repeated estimates from a single instrument. However, it is unlikely to be the main source of heterogeneity in prediction polling, where the estimates arise from different sources. 
...
\begin{align}
\left(\begin{matrix} Z_S \\ Z_{B_1}\\ \vdots \\ Z_{B_N} \end{matrix}\right) &\sim \mathcal{N}\left( 
%\left(\begin{matrix} 
...
\end{align}
This is a  \textit{distribution of information} that provides a covariance structure between the target quantity $Z_S$ and the observables $Z_{B_j}$. When combined with an appropriate \textit{link function} $g: \R \to \mathcal{Y}$, it  can be used to generate a variety of partial information models. This is illustrated in the next section.

In many forecasting setups such a relation may be more natural than assuming a 
This provides an alternative to the more classical approaches that assume 
...
%  = {\bf \Sigma}_{12} {\bf\Sigma}_{22}^{-1} \boldsymbol{Z}$ and $\bar{\Sigma} = 1 - {\bf\Sigma}_{12} {\bf\Sigma}_{22}^{-1} {\bf\Sigma}_{21}$. How this is utilized in different applications depends on the context. The decision-maker must choose a \textit{link function} that maps the observables to the information pool. The next section illustrates this for some common types of forecasts and outcomes. 
  ...
  \marginpar{Discuss (a sentence) oracular aggregator}
...
The revised yet equivalent estimation problem is
...
The final value of $\kappa$ is
 \begin{align}
\kappa_{out} = \argmax_{\nu \geq 0} \sum_{r=1}^R \ell\left( \Z_{(r)}, \mathcal{P}_{LSE}(h\left(\SS_{(r)}\right) : \nu)\right)
\label{kappa_out}
\end{align}
This approach, however, suffers from several drawbacks: a) it introduces a new tuning parameter $R$; b) it is computationally expensive as $\mathcal{P}_{LSE}(h\left(\SS_{(r)}\right) : \nu)$ must be called $R$ times for each candidate value $\nu$; and, most importantly, c) it can be unstable when $K$ is much smaller than $N$. This final drawback is particularly unfortunate because prediction-polls typically involve a large number of people giving estimates for several problems. 

%\marginpar{emphasize this more!!}

To address these drawbacks, this paper introduces a new selection procedure called \textit{conditional validation} (cov). This approach is particularly tailored for prediction and can be used for choosing any tuning parameter. In this paper, however, it is only used for estimating $\kappa$. To describe the procedure, recall that the revealed aggregator estimates $Z_S$ based on the full covariance matrix $\bSigma$ and the observations $Z_{B_j}$. 
%This matrix, however, is estimated only based on the observables $Z_j$. 
 Of course, the accuracy of this prediction cannot be measured until the actual outcome is observed. However, apart from the indexing Borel set and being unobserved, the variable $Z_S$ is theoretically no different to the observed $Z_{B_j}$'s. Therefore it is natural to let each of the $Z_{B_j}$'s in turn play the role of $Z_S$, predict its value based on the other $Z_{B_i}$'s for $i \neq j$ and the globally estimated $\mathcal{P}_{LSE}(h\left(\SS_{Z}\right) : \nu)$, and choose the value of $\nu$ that yields the best overall accuracy. Even though many accuracy measures could be chosen, this paper uses the conditional log-likelihood. That is, the final value of $\kappa$ is
  \begin{align}
\kappa_{cov} = \argmax_{\nu \geq 0} \sum_{j=1}^N \ell \left( \Z_{j}, \mathcal{P}_{LSE}(h\left(\SS_{Z}\right) : \nu) \big| \Z_i \text{ for } i \neq j\right), \label{kappa_cov}
\end{align}
where the log-likelihood is now conditional on $Z_{B_i}$'s for $i \neq j$.  In contrast to cross-validation, conditional validation requires only one call to $\mathcal{P}_{LSE}(h\left(\SS_{(r)}\right) : \nu)$ per candidate value $\nu$. Furthermore, it has no tuning parameters and remains rather stable when $N$ or $K$ is small. This is illustrated in the next section.
...
\label{binaryReal}
\subsubsection{Dataset and Examples}
...
All the events considered in this section have two possible outcomes; that is, $Y_k \in \{0,1\}$ for all $k = 1, \dots, K$. These outcomes are linked to the Gaussian information pool via the following function:
\begin{align*}
...
\end{align*}
where $\hat{\delta}_j$ represents the projected estimate of $\delta_j$. This procedure is repeated over a grid of $100$ candidate values $\nu$ between $10$ and $1,000$. The final value is chosen with the conditional validation technique discussed in Section \ref{condSelection}.


The revealed aggregator (\ref{RevAgg}) is benchmarked against common averaging aggregators: average probability, median probability, average probit-score, and average log-odds.  To avoid infinite log-odds
...
\end{align}
Given that $\E[X_j] = \mu_0$, the value of $\mu_0$ can be estimated with the sample mean of the $X_j$'s. The prior variance, however, is more challenging as all that is known is that it upper bounds the $\sigma_j^2$'s. Fortunately, as was mentioned earlier, the Gaussian model is invariant under scaling. Therefore the exact choice of $\sigma_0^2$ does not matter. To make this specific, suppose that $\varepsilon \sigma_0^2$ for some $\varepsilon > 0$ was used instead of $\sigma_0^2$. Then, $\Z \sim \mathcal{N}_N(\boldsymbol{0}, \bSigma_{22}/\varepsilon)$ and, by simple cancellation, $X''$ becomes exactly as in (\ref{revPoint}). Of course, for the sake of estimation, some $\sigma_0^2$ must be chosen. A numerically stable solution is to set $\sigma_0^2 = 2\max \{ \delta_j \}$.  
