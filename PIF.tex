...
% This section, however, focuses on predicting global events that are of particular interest to the Intelligence Advanced Research Projects Activity (IARPA).
 ...
 This paper develops a principled modeling framework for prediction polls. The focus, however, is on empirical work. Therefore the framework is illustrated on several real-world prediction polls. The first application considers a forecasting tournament initiated in 2011 by the Intelligence Advanced Research Projects Activity (IARPA) as a means to acquire accurate predictions on important geopolitical events. Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
% This success is largely credited to the
Since the start of the tournament, the GJP has recruited 
thousands of forecasters from professional societies, research centers, alumni associations, science bloggers, and word of mouth. 
...
These forecasters then estimated probabilities of the events specified by IARPA.  For instance, some of the questions were: ``Will Moody's issue a new downgrade on the long-term ratings for any of the eight major French banks between 30 July 2012 and 31 December 2012?'' and ``Will the Palestinian group Islamic Jihad significantly violate its cease-fire with Israel before 30 September 2012?". The forecasts given for these two questions are shown in Figure \ref{Examples}. Each question has a timeframe during which the participating forecasters made and updated their estimates as frequently as they liked without penalty. For instance, the question in Figure \ref{Example1} has a timeframe of $154$ days, ranging from 30 July 2012  to 31 December 2012. In general, the forecasters updated their estimates very infrequently, making the dataset extremely sparse. 
...
\centering
        ...
        \begin{subfigure}{0.49\textwidth}
                ...
                \caption{Will Moody's issue a new downgrade on the long-term ratings for any of the eight major French banks between 30 July 2012 and 31 December 2012?}
                                \label{Example1}
        \end{subfigure}
        \begin{subfigure}{0.49\textwidth}
                \includegraphics[width=\textwidth]{1160}
                \caption{Will the Palestinian group Islamic Jihad significantly violate its cease-fire with Israel before 30 September 2012?}
                                \label{Example2}
        \end{subfigure}             
        
        \caption{Two example problems. Given that neither event occurred, the forecasts converge towards the correct answer as the resolution day (the right most point) approaches. Problem (a) involved 522 forecasters, each making, on average, $6.4$ predictions. Problem (b), on the other hand, involved 521 forecasters with an average of $2.1$ predictions per forecaster.}
        \label{Examples}
\end{figure}
...
Once the predictions have been collected, the challenge is to combine them into a single consensus. This is typically done for the sake of decision-making and improved accuracy \citep{armstrong2}. Principled aggregation, however, requires an assumption about the source of heterogeneity among the forecasts. In particular, it is necessary to specify how the forecasts differ from the true value of the target quantity. For the past several decades, potentially due to the early forms of data collection, measurement error has served as the main source of heterogeneity. This approach has become the standard and is often applied in practice even when data variation is dominated  by causes besides error in measurement. For instance, the measurement error may be reasonable in modeling repeated estimates from a single instrument. However, it is unlikely to be the main source of heterogeneity in prediction polling, where the estimates arise from multiple, often widely different sources. 
...
The main contribution of this paper is a new source of forecast heterogeneity. In particular, differences are assumed to stem from information available to the forecasters and how they decide to use it. For instance, forecasters studying the same (or different) articles about a company may use separate parts of the information and hence report differing predictions on the company's revenue in the next quarter. The resulting framework, named the \textit{partial information framework}, is quite flexible yet intuitive. This is illustrated by deriving specific partial information models for several different types of forecasts, including point  and probability estimates. Under each model the optimal aggregator can be computed. These aggregators depend on an information structure for which an efficient estimation procedure is provided. The model is evaluated on several real world forecasting datasets. 
...
The paper is structured as follows.  The next section briefly reviews previous work on modeling forecasts. Section \ref{PIF} begins by describing the partial information framework at its most general level and then ends by introducing a close yet practical specification of the framework. This specification culminates in a distribution of information that incorporates the forecasters' information diversity through the covariance structure. Section \ref{estimator} derives a numerical procedure that efficiently estimates the information structure among the forecasters. Section \ref{empirical} illustrates the framework with applications to synthetic and real-world forecasts of different types of outcomes. The final section concludes with a summary and discussion of
future research.

\marginpar{be more specific about the data; second year etc.}
...
$\theta$, which can be interpreted as the prediction made by an ideal forecaster. This is then somehow measured by the individual forecasters with mean-zero idiosyncratic error.  Therefore, each forecast is an independent draw from a common
...
This framework has become standard practice possibly because the underlying probability model is both simple and very familiar.  Unfortunately, there are a number of disadvantages. First, average-based aggregators target the mean of forecasts. This, however, may not be the correct target. For instance, probability forecasting targets the event indicator that lies on the boundary of
the sample space. In such cases important properties such as unbiasedness cannot be expected from the resulting aggregators. Second, the standard assumption of conditional independence of the observations forces a specific and highly
...
Such approaches, however, do not reflect an actual model of forecasts and require a training set with known outcomes. For this reason, they are not discussed further in this paper. 


\subsection{Interpreted Signal Framework}
...
quantity to be predicted.  Differences among the 
predictions are ascribed to differing interpretation procedures.  For
example, consider two forecasters who visit a company and aim to predict its future revenue. One forecaster may carefully examine the company's technological status while while the other pays closer attention to what the manager tells them.  Even though the
forecasters receive the same information, they interpret it
differently and therefore are likely to arrive at different forecasts.
...
To the best of our knowledge, no previous work has discussed a formal framework that links the interpreted forecasts to their target quantity in an explicit yet flexible manner. Consequently, the interpreted signal framework, as proposed, has remained relatively abstract. The partial information framework, however, formalizes the intuition behind it, allows quantitative predictions, and provides a flexible construction that can be adopted to a broad range of forecasting setups. Therefore, the partial information framework is a statistical formalization of the interpreted signal framework. 
...
Consider $N$ forecaster and suppose Forecaster $i$ predicts $X_i$ for some (random) quantity of interest $Y$. The prediction $X_i$ is nothing but an estimator of $Y$. Therefore, as is the case with all estimators, 
...
its deviation from the truth can be broken down into two components: bias and noise. A forecaster is said to be conditionally unbiased if $\E[Y | X_i] = X_i$.  A stream of conditionally biased forecasts can, in principle, be corrected by
...
function $L(X_i, Y)$, which is a one-dimensional
...
self-fulfilling.  Forecasters can improve their long run performance via training and active
...
First, the construction  of the partial information framework begins with
%Consider
 a probability space $(\Omega, \F , \P)$ and a
measurable random variable $Y \in \F$ to be forecasted by $N$ forecasters. The probability measure $\P$ provides a non-informative (proper) prior belief on $Y$ and  reflects the \textit{basic information} known to all forecasters. 
...
Such a prior has been discussed extensively in the economics and game theory literature where it is usually known as the \textit{common prior} (see, e.g., \citealt{morris1995common, dawid1995coherent}). Even though this is a substantive assumption in the framework, specifying a prior distribution cannot be avoided as
long as the model depends on a probability space. This includes essentially any probability
model for forecast aggregation. How the prior is formulated depends on the problem context: it can be chosen explicitly by the decision-maker, computed based on historic data, or, as will be shown in Section \ref{empirical}, estimated directly from the forecasts. 
...
Forecaster $i$ reports $X_j = \E (Y \| \F_j)$, where $\F_j \subseteq \F$ is
the \textit{information set} used by the forecaster. This assumption is not in any way restrictive. To see this, recall that for a conditionally unbiased forecaster $\E (Y \| X_j) = X_j$. Under the given probability model, conditioning on $X_j$ is equivalent to conditioning on some $\F_j$, and hence $\E (Y \| X_j) =  \E (Y \| \F_j) = X_j$. Consequently, the assumption
...
assumptions on the structure of the information sets. To illustrate, suppose that the forecasters are asked to predict a sequence of values
$\{Y_1, Y_2 , \ldots \}$. The general framework only assumes that
Forecaster~$j$ uses some $\sigma$-field $\F_{ij}$ to make a forecast for $Y_i$.  Further aspects of the problem
...
sets $\{ \F_{ij} : i = 1 , 2, 3, \ldots \}$ are likely to form an
increasing sequence.  If the forecasts are public, it is reasonable 
to assume that $\F_{ij}$ contains all forecasts up to time $i-1$.
...
aggregator only through the forecasts $\{X_j : j = 1, \dots, N\}$. Given that $\F'$ generally cannot be constructed from these forecasts alone, no practically feasible aggregator can be expected to perform as well as $X'$.  Therefore, a more achievable benchmark is the \textit{revealed} aggregator $X'' := \E (Y \|
...
covariances $\Cov (Z_B , Z_{B'}) = |B \cap B'|$.  In other words, the
...
If the set $\mathcal{Y}$ represents the support of the target quantity $Y$, then the full white noise determines $Y := h(Z_S)$ via some surjective function $h: \R \to \mathcal{Y}$. 
...
The choice of the Gaussian distribution can be motivated both from the intuitive and technical points of view:
\begin{enumerate}[a)]
\item \textit{Intuitive:} Recall the interpreted signal
model of~\citet{broomell2009experts}. They assume that
Forecaster~$i$ forms an opinion based on $L_i (H_1 , \ldots , H_r)$,
where each $L_i$ is a linear function of cues
$H_1 , \ldots , H_r$ that influence the quantity of interest.  Proposing a linear model for subjective
...
\item \textit{Technical:} Both the multivariate and conditional Gaussian distributions are well-studied and have simple forms. This is particularly useful when deriving the revealed aggregator that depends on the distribution of the target quantity given the forecasters' predictions. 
...
replacing $S$ by $[0,u]$ and $B_j$ by $u B_j$. For more motivation and interpretation, see \cite{satopaamodeling}.
...
\end{align}
This  \textit{distribution of information} has several important properties: First, motivated by the partial information framework, it describes the covariance structure among all the information variables simply based on covariance of the observables $Z_{B_1}, \dots, Z_{B_N}$. Consequently, it is possible to regress $Z_S$ on $Z_{B_1}, \dots, Z_{B_N}$ without actually observing $Z_S$. Note that this is philosophically different and possibly more natural than the classical approaches that typically estimate a model parameter, such as the mean, and then use that directly as a prediction of $Z_S$. Second, information aggregation in practice requires the ability to estimate the value of one variable given the others. Under the multivariate Gaussian distribution (\ref{NExperts}) this is especially easy. In particular, if $\Z = (Z_{B_1}, \dots, Z_{B_N})'$, then $Z_S | \Z \sim \mathcal{N}\left(\bSigma_{12} \bSigma_{22}^{-1} \Z, 1-\bSigma_{12} \bSigma_{22}^{-1}\bSigma_{21}\right)$. This leads to closed-form revealed aggregator $X''$ in many applications. Third, the oracular aggregator can be emulated by appending (\ref{NExperts}) with a variable $Z_{B'}$, where $B' = \cup_{j=1}^N B_j$. This aggregator, however, is only useful in theoretical analysis and is therefore not discussed further in this paper. For more information see \cite{satopaamodeling} that use the oracular aggregator to analyze the empirical technique of extremizing, i.e., shifting an average aggregator closer to the nearer extreme (zero or one). Fourth, when combined with an appropriate \textit{link function} $g: \R \to \mathcal{Y}$, expression (\ref{NExperts}) can be used to generate a broad range  of partial information models. Section \ref{empirical} illustrates this by applying the Gaussian model to datasets with different types of outcomes. 
...
\label{estimator}
This section describes methodology for estimating the information structure $\bSigma_{22}$.  The predictions provide signal on $\bSigma_{22}$ as follows: a) the more similar two forecasters' predictions are, the higher the corresponding information overlap; and b) the more unlikely the forecast is according to the non-informative prior, the more informed the forecaster is likely to be. However, estimating $\bSigma_{22}$ in full generality based on a single prediction per forecaster is difficult, if possible at all. Therefore in this paper the forecasters are assumed to attend $K \geq 2$ related problems. For instance, the forecasters may participate in separate yet similar prediction problems or give repeated forecasts on a single recurring event. In such setups $\bSigma_{22}$ is likely to remain stable across problems and hence can be estimated based on multiple predictions per forecaster. See \cite{satopaamodeling} for aggregation of one-time events, i.e., setups with only a single prediction per forecaster. 
...
Estimating the information structure $\bSigma_{22}$ is equivalent to estimating a covariance matrix under constraints. Clearly, $0 \leq \rho_{ij} \leq \delta_i \leq 1$ for all $i,j$.  This, however, is not sufficient. The individual elements of $\bSigma_{22}$ are heavily linked to each other and form a highly constrained space. \cite{satopaamodeling} showed that a sufficient and necessary condition is $$\bSigma_{22} \in \COR_N :=  \conv\left\{
\boldsymbol{x}\boldsymbol{x}' : \boldsymbol{x} \in
\{0,1\}^N\right\},$$ where $\conv(\cdot)$ denotes the convex hull operator and $\COR_N$ is known as the correlation polytope (also known as the boolean quadric polytope by some authors). All elements of $ \COR_N$ are symmetric, positive semidefinite, and hence valid covariance matrices. 
%\citep{letchford2008binary}.  

These elements, however, differ largely in their sensitivity to numerical operations. This is important because an accurate yet highly sensitive estimate of $\bSigma_{12}$ can lead to imprecise aggregation. To make this more precise, notice that the Gaussian revealed aggregator includes the term $\bSigma_{12} \bSigma_{22}^{-1} \Z$. Re-express this term as $\w \Z$, where $\w$ is the solution to $\bSigma_{12} = \w \bSigma_{22}$. The rate at which this solution changes with respect to a change in $\bSigma_{12}$ depends on the condition number $\cond(\bSigma_{22}) := \lambda_{max}(\bSigma_{22}) / \lambda_{min}(\bSigma_{22})$, i.e., the ratio between the maximum and the minimum eigenvalues of $\bSigma_{22}$. If the condition number is very large, a small error in $\bSigma_{12}$ can cause a large error in $\w$. On the other hand, if the condition number is small, the $\bSigma_{22}$ is called \textit{well-conditioned} and error in $\w$ will not be much larger than the error in $\bSigma_{12}$. Therefore it is necessary to constrain $\cond(\bSigma_{22})$ during estimation. 
%A matrix with a low condition number is typically called \textit{well-conditioned}. 
...
Let $\boldsymbol{S}_Z$ denote the sample covariance matrix of $\Z$. Given that this is sufficient for $\bSigma_{22}$, the estimation procedure can then be written generally as
\begin{align}
\label{orig_problem}
\begin{split}
%\text{minimize } &  || \bSigma_{22} - \boldsymbol{S}_Z ||_F^2 \label{orig_problem}\\
\text{minimize } &  \ell \left(\bSigma_{22}, \boldsymbol{S}_Z\right) \\
\text{subject to } & \bSigma_{22} \in \COR_N, \\
& \cond(\bSigma_{22}) \leq \kappa,
\end{split}
\end{align}
where $\ell$ is a loss, a.k.a, the objective function, and $\kappa \geq 1$ is a fixed upper bound on the condition number. Both constraints in (\ref{orig_problem}) represent convex sets. Therefore,
...
%By introducing an auxiliary variable $u > 0$, the condition number constraint can be written as $ u I \preceq \bSigma_{22} \preceq \kappa u I$, where $A \preceq B$ denotes that $B - A$ is positive semidefinite.  This defines a convex region. 
 if $\ell$ is convex in $\bSigma_{22}$, the estimation task (\ref{orig_problem}) is a convex optimization problem. 
...
Typically the global optimum to such a problem can be found very efficiently. There are, however, two main difficulties: 
\begin{enumerate}[(i)]
...
\end{align*}
where $\diag(\M)$ is a vector of the diagonal elements of $\M$.  The correlation polytope is then relaxed with $\SD_N := \left\{ \M \in \mathcal{S}^N : h(\M) \in \mathcal{S}_{+}^{N+1} \right\}$. Given that $\bSigma := h(\bSigma_{22})$ is the covariance matrix of the multivariate Gaussian distribution (\ref{NExperts}), having $h(\bSigma_{22}) \in \SD_N$ can be viewed as the minimal requirement for $\bSigma_{22}$. It was discussed by \cite{laurent1997connections}, who showed that $\COR_N \subseteq \SD_N$ and called it, ``in some sense, the best possible relaxation of'' $\COR_N$. An equivalent relaxation has been extensively studied in the context of the well-known Max-Cut problem, where it has been found to be surprisingly strong both in theory and practice (see, e.g., \citealt{wolkowicz2000handbook}). 
...
The relaxed version of (\ref{orig_problem}) then becomes
\begin{align}
\label{relax_problem}
\begin{split}
%\text{minimize } &  || \bSigma_{22} - \boldsymbol{S}_Z ||_F^2 \label{orig_problem}\\
...
Given that the feasible region of this problem has a simple form, it responds to difficulty (\ref{compProblem2}) from Section \ref{generalEstimation}. The next subsection addresses difficulty (\ref{compProblem1}) by selecting a loss function that allows the dimension of the problem to be reduced largely. 
...
The information structure $\bSigma_{22}$ is a parameter of an explicit likelihood. Therefore estimation naturally begins with the maximum likelihood approach (MLE) under which the loss $\ell \left(\bSigma_{22}, \boldsymbol{S}_Z\right) $ is equal to the Gaussian likelihood. Unfortunately, this loss function is not convex in $\bSigma_{22}$. Consequently, only a locally optimal solution is guaranteed with standard optimization techniques. Second, based on preliminary simulations, the MLE does not perform very well, especially when $K$ is much smaller than $N$. Third, it is not clear whether the dimension of this form can be reduced. \cite{won2006maximum} consider the MLE of the Gaussian covariance matrix under a condition number constraint. They are able to transform the original problem with $\binom{N+1}{2}$ variables to an equivalent problem with only $N$ variables, namely the eigenvalues of $\bSigma_{22}$. This transformation, however, requires an orthogonally invariant problem. Given that relaxed constraint $\bSigma_{22} \in \SD_N$ is not orthogonally invariant, the same dimension-reduction technique cannot be applied. Instead, the MLE must be computed with the $\binom{N+1}{2}$ variables, making estimation slow for small $N$ and undoable for moderate to large $N$. For these reasons the MLE is not discussed further in this paper.
...
An alternative that permits dimension-reduction is to project an unconstrained estimator onto the feasible region. This approach, sometimes known as the least squares estimation, has been studied extensively by many authors in the past (see, e.g., \citealt{soloveychik2013covariance, barton1997structured, henrion2012projection, malick2004dual, hu1991numerical}). 
...
The dimension reduction is facilitated by a change of variables and projecting $h(\boldsymbol{S}_Z)$ instead of $\boldsymbol{S}_Z$. 
%The functional invariance of the MLE shows that $h(\boldsymbol{S}_Z)$ is an unconstrained MLE of  $h(\bSigma_{22}) = \bSigma$. 
The revised estimation problem is
...
\end{split}
\end{align}
where $\Tr(\cdot)$ is the trace operator and $||\cdot||_F^2$ is the squared Frobenius norm $||\M||_F^2 = \Tr(\M'\M)$. Even though many other norms could be used, for the sake of simplicity, this paper only considers the Frobenius norm. The linear constraints in (\ref{lse_problem}) maintain the structure induced by $h(\cdot)$. In particular, $b_1 = 1$ and $\A_1$ is filled with zeros except $a_{11} = 1$. For $j \geq 2$, $b_j = 0$ and $\A_j$ is filled with zeros except $a_{jj} = 1$ and  $a_{j1} = a_{1j} = -1/2$. Given that  $\bSigma_{22}$ is a principal sub-matrix of $\bSigma$, the condition $\bSigma \in \mathcal{S}_{+}^{N+1}$ implies $\bSigma_{22} \in \mathcal{S}_{+}^{N}$. Furthermore, by Cauchy's Interlace Theorem, $\lambda_{min}(\bSigma) \leq \lambda_{min}(\bSigma_{22})$ and $\lambda_{max}(\bSigma_{22}) \leq \lambda_{max}(\bSigma)$ such that $\cond(\bSigma_{22}) \leq \cond(\bSigma) \leq \kappa$. 
...
The first step in solving problem (\ref{lse_problem}) expresses the feasible region as an intersection of two sets, namely 
\begin{align*}
...
First, projecting a matrix $\M$  onto $\mathcal{C}_{lin}$ is a linear map. To make this more specific, let $\m = \vec(\M)$ be a column-wise vectorization of $\M$. 
...
If $\A$ is a matrix with the $j$th row equal to $\vec(\A_j)$ for $j = 1, \dots, N+1$, then the  linear constraints in (\ref{relax_problem}) can be expressed as $\A\m =  \boldsymbol{e}_1$, where $\boldsymbol{e}_1$ is the first standard basis vector. The projection of  $\M$ onto $\mathcal{C}_{lin}$ then is $\vec^{-1}(\m + \A' (\A \A')^{-1}(\boldsymbol{e}_1 - \A\m))$. This expression simplifies significantly by close inspection. In particular, the projection is equivalent to setting $m_{11} = 1$ and replacing $m_{i1}$, $m_{1i}$, and $m_{ii}$ by their average $(m_{ii} + m_{i1} + m_{1i})/3$, for $i = 2, \dots, N+1$. Denote this projection with the operator $\mathcal{P}_{lin}(\cdot)$. 

Second, \cite{tanaka2014positive} re-express the projection of $\M$ onto $\mathcal{C}_{sd}$ as a univariate optimization problem. To  describe this approach, consider the spectral decomposition $\M = \Q \Diag(l_1, \dots, l_N) \Q'$, where $\Diag(\x)$ is a diagonal matrix with $\x$ on the diagonal, and the univariate function 
\begin{align*}
g(\mu) &= \sum_{i=1}^{N} \left[ \left(\mu-l_i\right)^2_+ + \left(l_i - \kappa\mu\right)^2_+ \right],
\end{align*}
where $(\cdot)_+$ is the positive part operator. The function $g(\mu)$ can be minimized very efficiently by solving a series of smaller convex problems, each with a closed form solution. Complete technical details of this are given in the Appendix.
...
%\end{align*}
for $j = 1, \dots, N$,  then $\Q \Diag(\lambda_1^*, \dots, \lambda_{N}^*) \Q$ is the projection of $\M$ onto $\mathcal{C}_{sd}$. Call this projection $\mathcal{P}_{sd}(\cdot : \kappa)$. 


 Algorithm \ref{proj_algo} uses these projections to solve (\ref{lse_problem}). Given that all the steps are in closed-form, except the univariate subroutine in $\mathcal{P}_{cond}(\cdot:\kappa)$, each iteration is very fast.  Overall time to convergence, of course, depends on the choice of the stopping criterion. Many intuitive criteria are possible. Given that $\bSigma_D \in \mathcal{C}_{sd}$ and $\bSigma_C \in \mathcal{C}_{lin}$, the stopping criterion $\max \{(\bSigma_{D}-\bSigma_{C})_{ij}^2\} < \epsilon$ suggests that the final projection is in $\mathcal{C}_{sd}$ and close to $\mathcal{C}_{lin}$ in every direction. Based on our experience, the algorithm converges quite quickly. For instance, our implementation in C++ solves (\ref{lse_problem}) for $\epsilon = 10^{-5}$ and $N = 100$ in less than a second on a 1.7 GHz Intel Core i5 computer.  This suggests that Algorithm \ref{proj_algo} addresses difficulty (\ref{compProblem1}). For the rest of the paper, denote the projection onto the intersection $\mathcal{C}_{sd} \cap \mathcal{C}_{lin}$ with $\mathcal{P}_{LSE}(\cdot : \kappa)$. 
...
\begin{algorithm}[t]
\caption{This procedure projects the point $h(\boldsymbol{S}_Z)$ onto the intersection $\mathcal{C}_{sd} \cap \mathcal{C}_{lin}$. Denote this projection with $\mathcal{P}_{LSE}(\cdot : \kappa)$. }
\label{proj_algo}
\begin{algorithmic}[1]
...
\State $\bSigma_{B} \hspace{0.5em} \leftarrow \mathcal{P}_{sd}(\bSigma_{A} : \kappa)$
\State $\bSigma_{C} \hspace{0.5em} \leftarrow \mathcal{P}_{lin}(\bSigma_{B})$
\State $\bSigma_{D} \hspace{0.5em} \leftarrow \mathcal{P}_{sd}(\bSigma_{C} : \kappa)$
...
\label{condSelection}
%Given that Algorithm \ref{proj_algo} depends on $\kappa$, a principled approach to selecting such a value should be proposed. 
...
Previous literature has mentioned an approach to choosing the value of $\kappa$.
% has been discussed in the previous literature. 
In particular,  \cite{won2006maximum} discuss maximum likelihood estimation under a condition number constraint and propose choosing the value of $\kappa$ that maximizes the expected predictive log-likelihood. That is, if $\Z_1, \dots, \Z_K,  \tilde{\Z}\stackrel{i.i.d.}{\sim} \mathcal{N}_{N}(\boldsymbol{0}, \bSigma_{22})$, they choose $\kappa = \argmax_{\nu \geq 0} \E \left\{ \E_{\tilde{\Z}} \left(  \ell(\tilde{\Z},  \mathcal{P}_{LSE}(h(\SS) : \nu)) \right) \right\}$, where $\ell(\cdot)$ denotes the log-likelihood  and $\SS$ is the sample covariance matrix computed only based on $\Z_1, \dots, \Z_K$. They approximate the expected predictive log-likelihood with cross validation. This partitions the data $\Z$ into $R$ subsets $\Z = \left(\Z_{(1)}, \dots, \Z_{(R)}\right)'$. During the $r$th iteration, $\Z_{(r)}$ functions as $\tilde{\Z}$ and the remaining $R-1$ subsets are used for computing the sample covariance matrix, denoted  with $\SS_{(r)}$. 
The chosen value of $\kappa$ is
 \begin{align}
\kappa_{out} = \argmax_{\nu \geq 0} \sum_{r=1}^R \ell\left( \Z_{(r)}, \mathcal{P}_{LSE}(h\left(\SS_{(r)}\right) : \nu)\right)
\label{kappa_out}
\end{align}
This approach, however, suffers from several drawbacks: a) it introduces a new tuning parameter $R$; b) it is computationally expensive as $\mathcal{P}_{LSE}(h\left(\SS_{(r)}\right) : \nu)$ must be called $R$ times for each candidate value $\nu$; and, most importantly, c) it can be unstable when $K$ or $N$ is small. The last drawback is particularly unfortunate because in prediction polling
%typically involve a large number of people attending relatively few problems. T
 the number of responses per forecaster is often kept low for the sake of avoiding responder fatigue and hence the potential of reduced  estimation accuracy. 

%\marginpar{emphasize this more!!}

To address these drawbacks, the current paper introduces a new selection procedure called \textit{conditional validation}. This approach is particularly tailored for prediction and can be used for choosing any tuning parameter in a partial information model. To describe the procedure, recall that the revealed aggregator $X''$ uses an estimate of the full covariance matrix $\bSigma$ to predict  $Z_S$ from the $Z_{B_j}$'s. 
%This matrix, however, is estimated only based on the observables $Z_j$. 
 Of course, the accuracy of this prediction cannot be known until the actual outcome is observed. However, apart from the indexing Borel set and being unobserved, the variable $Z_S$ is theoretically no different to the observed $Z_{B_j}$'s. Therefore it is natural to let each of the $Z_{B_j}$'s in turn play the role of $Z_S$, predict its value based on $Z_{B_i}$ for $i \neq j$, and choose the value of $\nu$ that yields the best overall accuracy. Even though many accuracy measures could be chosen, this paper uses the conditional log-likelihood. Therefore the chosen value of $\kappa$ is
  \begin{align}
\kappa_{cov} = \argmax_{\nu \geq 0} \sum_{j=1}^N \ell \left( \Z_{j}, \mathcal{P}_{LSE}(h\left(\SS_{Z}\right) : \nu) \big| \Z_i \text{ for } i \neq j\right), \label{kappa_cov}
\end{align}
where the log-likelihood is now conditional on $Z_{B_i}$'s for $i \neq j$. This imitates the idea behind cross-validation but, instead of predicting across rows (observations), it predicts across columns (variables).  In contrast to cross-validation, however, conditional validation requires only one call to $\mathcal{P}_{LSE}(h\left(\SS_{(r)}\right) : \nu)$ per candidate value $\nu$. Furthermore, it has no tuning parameters and remains rather stable when $N$ or $K$ is small. This is illustrated in the next section.

%\marginpar{this must be re-written better. This can be viewed as column-wise CV while the regular CV is row-wise.}


Unfortunately, both (\ref{kappa_out}) and (\ref{kappa_cov}) are non-convex in $\nu$.  However, as was mentioned before, Algorithm \ref{proj_algo} is fast for moderately sized $N$. Therefore $\kappa$ can be chosen efficiently over a  grid of candidate values.  In practice an extremely accurate selection of $\kappa$ is hardly necessary. Therefore, especially when $N$ is large, computation time can be saved largely by choosing $\kappa$ with a cruder tolerance.  
...
\label{empirical}
This section evaluates the Gaussian model both on synthetic and real-world forecasting data. The discussion illustrates how the Gaussian model can be applied to forecasting setups with different types of outcomes and forecasts. For these derivations, it is helpful to keep in mind that an instance of the Gaussian model has the following components: the information variables $Z_S, Z_{B_1}, \dots, Z_{B_N}$, a target quantity $Y  \in \mathcal{Y}$, the forecasts $X_j = \E[Y | Z_{B_j}]$, and a surjective link function $g: \R \to \mathcal{Y}$ such that $Y = g(Z_S)$. 
%\end{enumerate}
%
%\begin{align*}
%%\text{Model Instance} = 
...
\subsection{Simulation Study: Point Forecasts}
\label{simulation}
Consider the most basic setup where the link function $g$ is the identity. Consequently,  $Y = g(Z_{S}) = Z_S$ and $X_j = \E[Y | Z_{B_j}] = Z_{B_j}$ for $j = 1, \dots, N$. Suppose the forecasters attend $K$ problems. Collect the information variables associated with problem $k$ into a vector $\Z_k = (Z_{S,k}, Z_{B_{1},k}, \dots, Z_{B_N, k})'$. For each problem the simulation first draws $\Z_k \stackrel{i.i.d.}{\sim} \mathcal{N}_{N+1}(\boldsymbol{0}, h(\bSigma_{22}))$. The  aggregators then observe $\Z_k^{-}$, where $\Z_k^{-} = (Z_{B_{1},k}, \dots, Z_{B_N, k})'$ holds only the forecasters' information variables such that $\Z_k = (Z_{S,k}, {\Z_k^{-}}')'$, and predict $Z_{S,k}$ based on this information. The process is repeated a $2,000$ times under different values of $K$, ranging from $5$ to $35$ with constant increments of $5$. The number of forecasters $N$ is fixed at $20$. 


Before a random $\Z_k$ can be generated, a covariance matrix $\bSigma_{22} \in \COR_N$ must be specified. This can be done by first drawing $\delta_i \sim  \mathcal{U}(0,1)$ for all $i = 1, \dots, N$, and then setting $\rho_{ij} = \delta_i \delta_j$. To verify that such a structure is always in $\COR_N$, approximate the full information with a finite set of $L$ elements. Forecaster $j$'s information can then be  described by a binary vector $\v_j$ whose $L$ entries indicate which elements of the full information the forecaster knows. Suppose this vector is constructed by making $L$ independent draws from a Bernoulli distribution with success probability $\delta_j$. If $\V$ is a $N \times L$ matrix with rows equal to the forecasters' information vectors, the corresponding (random) information structure is $\V \V' / L$, which, by construction, is always in $\COR_N$. Given that $\COR_N$ is a convex set, the expectation of  $\V \V' / L$ is also in $\COR_N$. The diagonal and off-diagonal elements of this expectation are the desired $\E[\v_j' \v_j] = \delta_i$ and $\E[\v_i' \v_j] = \delta_i \delta_j$, respectively. This produces a broad range of information structures, both well- and ill-conditioned. In fact, if any $\delta_j = 0$, the resulting $\bSigma_{22}$ is singular.  To avoid this, the $\delta_j$'s are generated uniformly at random from the somewhat more realistic interval $[0.1, 0.9]$.
...
\vspace{-3.5em}
        ...
        \begin{subfigure}{0.5\textwidth}
                ...
                \caption{Condition numbers of the different estimators}
                                \label{SigmaInv}
        \end{subfigure}
        \caption{Estimation of the information structure and its inverse. Both are important for accuracy prediction of $Z_S$. The vertical line represents the number of forecasters, $N$, that has been fixed to $20$.}
        \label{SigmaEstimation}
\end{figure}

In this forecasting setup, the revealed aggregator for problem $k$ is $X_k'' = \diag(\bSigma_{22})' \bSigma_{22}^{-1}\Z_k^{-}$. This, of course, requires an estimate of $\bSigma_{22}$. This subsection considers $X_k''$ under four different versions of $\bSigma_{22}$ in addition to two aggregators based on the classical measurement error model:
\begin{enumerate}[i)]
\item $X_k''$ with $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$, where $\kappa_{cov}$ is found over a grid of $10$ values from $10$ to $1,000$. 
\item $X_k''$ with  $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$, where $\kappa_{out}$ is found over the same grid of values as $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$. The cross-validation uses five folds, i.e. $R = 5$. 
\item $X_k''$ with the sample covariance matrix $\SS_Z$. Given that $\SS_Z$ is singular when $K<N$, its inverse is computed with the (Moore-Penrose) generalized inverse.
\item $X_k''$ with the true information structure $\bSigma_{22}$. 
\item The average forecast.
\item The median forecast.
\end{enumerate}

Recall from Section \ref{estimator} that accurate prediction stems both from a precise estimate of $\bSigma_{22}$ and a low condition number. This allows different strategies for achieving good accuracy. In particular, the two selection procedures discussed in Section \ref{condSelection} make slightly different tradeoffs between a precise estimate of $\bSigma_{22}$ and its condition number. This is illustrated in  Figure \ref{SigmaEstimation}. First, Figure \ref{Sigma} examines how $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$, $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$, and $\SS_Z$ capture the true $\bSigma_{22}$. 
Even though all estimators find $\bSigma_{22}$ more accurately as $K$ grows, $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ and $\SS_Z$ improve at a higher rate than $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$.  In fact, if $K > N$, $\SS_Z$ and $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ perform better than $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$. Next,  Figure \ref{SigmaInv}  presents the corresponding condition numbers. For the sake of keeping the scale manageable, $\cond(\SS_Z)$ has been omitted. Notice that $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{out}))$ increases while $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{cov}))$ generally decreases as $K$ grows larger. In fact, when $K > N$,  $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{cov}))$ is smaller than $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{cov}))$. From the prediction perspective, this makes conditional-validation more forgiving towards error in the estimated $\bSigma_{22}$. Therefore, while $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$ incorporates the actual prediction process and looks for a fine balance between a precise estimate of $\bSigma_{22}$ and a low condition number, $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ is unaware of the prediction process and simply focuses on estimating $\bSigma_{22}$ as accurately as possible. These two strategies lead to different predictive behavior.
...
\begin{figure}[t!]
\centering
\hspace*{2em} 	\includegraphics{legendSim2.pdf} % requires the graphicx package
\vspace{-3.5em}

                \includegraphics[width=0.5\textwidth]{simResX.pdf}
        \caption{The accuracy to predict $Z_S$. The vertical line represents the number of forecasters, $N$, that has been fixed to $20$.}
        \label{ZsEstimation}
\end{figure}

 Figure \ref{ZsEstimation} shows the average squared error loss of the competing aggregators in predicting $Y$. Given that $Y \sim \mathcal{N}(0,1)$, the loss of the prior mean $\E[(Y-0)^2] = 1.0$ can be considered as a reasonable upper bound in prediction error. The revealed aggregator $X''$ with the true information structure $\bSigma_{22}$, on the other hand, is optimal and hence provides the corresponding lower bound. Under most values of $K$, the revealed aggregator $X''$ with $\SS_Z$ received a loss well above $1.0$ and is therefore not included in the figure. Overall, the average and median perform very similarly, with average loss fixed at around $0.26$. This is uniformly well above the loss of the revealed aggregator. The two versions, namely  $X''$ with $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$ and $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ perform very similarly when $K \geq N$. They monotonically tend close to the optimal at around $0.03$ as $K$ grows larger. However, when $K < N$, conditional validation is more robust and clearly yields better results. For this reason, only $\kappa_{cov}$ is considered in the following subsections that apply the Gaussian model to real-world forecasting data. 
...
\label{binaryReal}
\subsubsection{Aggregation Accuracy}
...
% 
\textit{Data.} Since 2011 the Good Judgment Project (GJP) has recruited thousands of forecasters to make probability estimates on different geopolitical events.  This subsection analyzes the estimates made during the second year of the project. During this year the forecasters made predictions for $78$ events, each with two possible outcomes. For instance, some of the questions were: ``Will Moody's issue a new downgrade on the long-term ratings for any of the eight major French banks between 30 July 2012 and 31 December 2012?'' and ``Will the Palestinian group Islamic Jihad significantly violate its cease-fire with Israel before 30 September 2012?''. These examples are illustrated in Figure \ref{Examples}. %Since 2011 t%Before the forecasts can be aggregated, however, several practical matters must be discussed. First, 
In general, each event is associated with a timeframe, defined as the number of days between the first day of forecasting and the anticipated resolution day. 
...
The events had very different timeframes, ranging from 12 days to 519 days with a mean of 185.4 days.
% 
%  For instance, the events in Figures \ref{Example1} and \ref{Example2} have timeframes of 154 and 52 days, respectively. 
 The outcome of the event is typically the most uncertain at the beginning and then becomes more certain towards the resolution date. The forecasters made and updated their predictions as frequently as they liked. Not all forecasters made predictions for all the events. Furthemore, the forecasters generally updated their forecasts infrequently, resulting into a very sparse dataset. 
 
The forecasters knew that their probability estimates would be assessed for accuracy using Brier scores, i.e., the squared distance between the probability forecast and the event indicator that equals $1.0$ or $0.0$ depending on whether the event happened or not, respectively. This incentivized them to report their true beliefs instead of attempting to game the system. In addition to receiving \$150 for meeting minimum participation requirements that did not depend on prediction accuracy, the forecasters received status rewards for their performance via leader-boards displaying Brier scores for the top $20$ forecasters.



This data raises two issues: First, given that the current paper does not focus on modeling dynamic data, only forecasts made within some time interval are considered. The goal is to choose an interval during which predicting the correct outcome is neither trivial nor extremely difficult because in such cases the competing aggregators are unlikely to present meaningful differences. Second, the data is very sparse, yet the Gaussian model  requires an unconstrained estimator $\SS_Z$. Under a large number of missing observations, one option is to simply use all the complete pairs of observations to estimate the corresponding covariance.  This requires any two forecasters to make predictions on at least two common problems. However, given that evaluating different techniques to handle missing values is well outside the scope of this paper, only the hundred most active forecasters are considered. More specifically, the analysis focuses on two scenarios:
 \begin{enumerate}[a)]
\item \textit{Lower Uncertainty (LU):} Consider each forecaster's most recent forecast during $30-60$ days before the anticipated resolution day. The resulting dataset has $60$ events of which $13$ occurred. There are $841$ forecasters attending, on average, $11.3$ problems, with a range of $1$ to $55$ problems. The top $100$ most active forecasters attended, on average, $34.9$ problems, with a range of $25$ to $55$ problems. 
 
\item \textit{Higher Uncertainty (HU):} Consider each forecaster's most recent forecast during  $90-120$ days before the anticipated resolution day. The resulting dataset has $49$ events of which $10$ occurred. There are $732$ forecasters attending, on average, $7.7$ problems, with a range of $1$ to $46$ problems.  The top $100$ most active forecasters attended, on average, $24.0$ problems, with a range of $14$ to $46$ problems. 
 \end{enumerate}
Another study considering the $60-90$ days before the anticipated resolution day was also conducted. The results, however, were qualitatively very similar to the results under LU and HU. Therefore, for the sake of brevity, this middle scenario is not included in the final version of the paper.  



Recall that the partial information framework is designed for noise-reduction. Therefore the forecasts are assumed to be conditionally unbiased, i.e., \textit{calibrated}, as this condition is more often called in the probability forecasting literature. Unfortunately, human experts are generally known to lack calibration. According to some simple performance metrics, this is also the case with the GJP data. Even though the level of calibration can be somewhat improved via pre-processing mechanisms, such as Platt scaling \citep{platt1999probabilistic} or Isotonic regression \citep{zadrozny2001obtaining}, these techniques require a training set with known outcomes and hence are not in the scope of this paper. Consequently, the GJP forecasts are not pre-processed. The results, however, suggest that the Gaussian model remains robust and is able out-perform competing aggregators even when the forecasts are not extremely well-calibrated. 
...
\vspace{2em}
\noindent
\textit{Model.} All the events considered in this section have two possible outcomes; that is, $Y_k \in \{0,1\}$ for all $k = 1, \dots, K$. These outcomes are linked to the Gaussian information pool via the following function:
\begin{align*}
Y_k &= g(Z_{S,k}) = \begin{cases}
1 & \text{ if } Z_{S,k} > t_k\\
0 & \text{ otherwise},\\
\end{cases}
\end{align*}
where $t_k \in \R$ is some threshold value. Therefore the link function $g(\cdot)$ is simply the indicator function $\one_{A_k}$ of the event $A_k = \{Z_{S,k} > t_k\}$. 
%Overall, the events are quite different. Therefore it is important to allowing the threshold $t_k$ to change across problems. 
The corresponding probability forecasts $X_{j,k} \in [0,1]$ are
\begin{align*}
X_{jk} &= \E[Y_k | Z_{B_j,k}] = \Phi\left( \frac{Z_{B_j,k} - t_k}{\sqrt{1-\delta_j}}\right),
\end{align*}
where $\Phi(\cdot)$ is the CDF of a standard Gaussian distribution. Furthermore, if $\Z_k = (Z_{B_1, k}, \dots, Z_{B_N, k})'$ collects the forecasters' information variables, then the revealed aggregator $X_k'' \in [0,1]$ for event $k$ is
\begin{align}
...
\vspace{2em}
\noindent
\textit{Estimation.} All the unknown parameters of this model can be estimated. The first step is to specify an unconstrained estimate of $\bSigma_{22}$. If the $t_k$'s do not change much, a reasonable and simple estimate can be obtained  by transforming the sample covariance matrix  $\SS_P$ of the probit scores $P_{jk} := \Phi^{-1}(X_{jk})$; that is,  if $\D := \Diag( \dd ) \Diag(\one + \dd)^{-1}$, where $\dd = \diag(\SS_P)$, then an unconstrained estimator of $\bSigma_{22}$ is given by $\SS_Z = (\I_N-\D)^{1/2} \SS_P (\I_N-\D)^{1/2}$. Next, choose a candidate value $\nu$ and project $\SS_Z$ onto the feasible region via $\mathcal{P}_{LSE}(\cdot : \nu)$. To estimate the thresholds $t_k$, let $\PP_k = (P_{1k}, \dots, P_{Nk})'$ and observe that $-\Diag(\one-\diag(\bSigma_{22}))^{1/2}\PP_{k} \sim \mathcal{N}_N(t_k \one_N, \bSigma_{22})$.
...
% observing that $P_{jk} := -\Phi^{-1}(X_{jk})\sqrt{1-\delta_j} \sim \mathcal{N}(t_k, \delta_j)$. 
 The threshold $t_k$ is then estimated by the following precision-weighted average: 
\begin{align*}
%\hat{t}_k &= -\frac{\sum_{j=1}^N \sqrt{1-\hat{\delta}_j}P_{jk}/\hat{\delta}_j}{\sum_{j=1}^N 1/\hat{\delta}_j},
\hat{t}_k &= -\frac{\PP_{k}' \Diag(\one-\diag(\hat{\bSigma}_{22}))^{1/2} \hat{\bSigma}_{22} \one}{\one' \hat{\bSigma}_{22} \one},
\end{align*}
where $\hat{\bSigma}_{22}$ represents the current projection of $\SS_Z$. This procedure is repeated over a grid of $100$ candidate values $\nu$ between $10$ and $1,000$. The final value is chosen with the conditional validation technique discussed in Section \ref{condSelection}.

\vspace{2em}
\noindent
\textit{Analysis.} The revealed aggregator (\ref{RevAgg}) is benchmarked against common averaging aggregators: average probability, median probability, average probit-score, and average log-odds.  To avoid infinite log-odds
and probit scores, extreme forecasts $X_{jk} = 0$ and $1$ were
censored to $X_{jk} = 0.001$ and $0.999$, respectively. Aggregation accuracy is measured with the mean Brier score (BS) that for an aggregate forecast $X_k$ is defined as
%Consider $K$
...
\text{BS} &= \frac{1}{K} \sum_{k=1}^K \left(X_k- \one_{A_k}\right)^2.
 \end{align*}
This score takes values within the unit interval with lower values indicating higher accuracy. The evaluation is performed via repeated subsampling of the forecasters: choose $N$ forecasters uniformly at random and then aggregate their forecasts. This is repeated 1,000 times with $N = 5, 10, \dots, 50$ forecasters. The simulation study is performed separately for the HU and LU setups. 

\marginpar{Explain here that this is evidence that information diversity is more important.}

\begin{figure}
\vspace{-2em}
\centering
\hspace*{0em} 	\includegraphics[width=\textwidth]{legendReal.pdf} % requires the graphicx package
\vspace{-3.5em}

        \centering
        \begin{subfigure}{0.45\textwidth}
                \includegraphics[width=\textwidth]{realFirst.pdf}
                \caption{Higher Uncertainty}
                                \label{highUnc}
        \end{subfigure}
        \begin{subfigure}{0.45\textwidth}
                \includegraphics[width=\textwidth]{realLast.pdf}
                \caption{Lower Uncertainty}
                                \label{lowUnc}
        \end{subfigure}             
...
\marginpar{Change the legend to hold X''}


Figure \ref{simReal} shows the average BS over the 1,000 iterations under both setups. Notice that the scores in Figure \ref{highUnc} are uniformly lower than in Figure \ref{lowUnc}. This is reflection of the higher uncertainty in the HU setup. Both setups, however, tell a similar story about the relative accuracies of the competing aggregators. First, the averaging aggregators rank in the typical order (from worst to best): average probability, median probability, average probit, and average log-odds. Second, the averaging aggregators cannot utilize the increasing number of forecasters and hardly change once $N \geq 10$. Third, even though the revealed aggregator performs worse than the averaging aggregators under small $N$, it is able to collect information more efficiently as $N$ grows. Intuitively, this is the kind of behavior one would expect from an aggregator. Consequently,  $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$  achieves the best score for  $N \geq 15$, with the performance advantage growing in $N$. Fourth, even though $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ outperforms $\mathcal{P}_{LSE}(\cdot : \kappa_{in})$ for small $N$, this advantage diminishes as $N$ grows. In fact, for $N \geq 30$,  $\mathcal{P}_{LSE}(\cdot : \kappa_{in})$ outperforms $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$. This suggests the use of the computationally less demanding $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ under large $N$. 
...
\cite{satopaamodeling} show that averaging-like aggregators work well when the forecasters have very similar information sets. The extent to which the revealed aggregator outperforms the averaging-aggregators suggests that the GJP forecasters exhibit high information diversity. This is not surprising because the individuals are quite different. Furthermore, the GJP pre-assigned some forecasters to make predictions in isolation and others to work in teams. After the first year of the tournament, the top 1\% forecasters were selected to the elite group of ``super-forecasters''. During the second year these super-forecasters worked in teams to make highly accurate predictions on the same events as the rest of the forecasters. Many of these characteristics are suggestive of the information overlap. For instance, super-forecasters should have the highest $\delta_j$'s and forecasters in the same team should have a relatively high $\rho_{ij}$. This section examines the extent to which our prior knowledge on information overlap coincides with the estimated $\bSigma_{22}$. 
...
%	\vspace{0.8em}
        ...
        \caption{This figure shows the estimated information structure $\bSigma_{22}$ for all the forecasters in the dataset and for the five most active forecasters. Notice that the super-forecasters generally have more information than the forecasters working in isolation.}
        \label{SynthPlot}
\end{figure}

%% 4470 and 1119 and  2763, 5265, and 4916

The 100 forecasters considered in the previous subsection consist of 33 individuals predicting in isolation, 33 forecasting team-members (across 22 teams), and 34 super-forecasters (across 5 teams).  Figure \ref{top5} displays the estimated $\bSigma_{22}$ for the five most active forecasters. This group involves two forecasters working in isolation ($I_1$ and $I_2$) and three super-forecasters ($S_1$, $S_2$, and $S_3$). Only the super-forecasters $S_1$ and $S_2$ are in the same team and hence have a relatively higher information overlap. Overall, the three super-forecasters are more informed than the non-super-forecasters. Such a high level of information unavoidably leads to higher information overlap. In the contrary, the non-super-forecasters' information sets are smaller and hence  can fit inside the unit interval with a smaller overlap. 


This pattern generalizes to the entire group of forecasters.  Figure  \ref{top100} displays the estimated $\bSigma_{22}$ for all 100 forecasters. The information structure has been ordered with respect to the diagonal such that the most (least) informed forecasters appear on the right (left). Furthermore, a colored rug has been appended on top of the plot. This rug shows whether each forecaster worked in isolation (black), in a non-super-forecaster team (red), or in a super-forecaster team (blue). Observe that the super-forecasters are mostly situated on the right among the most informed forecasters. The average estimated $\delta_j$ among the super-forecaster is $0.80$.  On the other hand, the average estimated $\delta_j$ among the individuals working isolation or in non-super-forecaster teams were $0.47$ and $0.50$, respectively. Therefore working in a team makes the forecasters' predictions more informed. 
...
\begin{align}
X'' =  \E\left[Y  | \F''\right] =  \E\left[Y | \Z\right] &= {\bf\Sigma}_{12} {\bf\Sigma}_{22}^{-1} \Z \sigma_0 + \mu_0 \label{revPoint}
\end{align}
Given that $\E[X_j] = \mu_0$, the value of $\mu_0$ can be estimated with the sample mean of the $X_j$'s. The prior variance, however, is more challenging as all that is known is that it upper bounds the $\sigma_j^2$'s. Fortunately, the revealed aggregator is not affected by this choice. To make this specific, suppose that $\varepsilon \sigma_0^2$ for some $\varepsilon > 0$ was used instead of $\sigma_0^2$. Then, $\Z \sim \mathcal{N}_N(\boldsymbol{0}, \bSigma_{22}/\varepsilon)$ and, by simple cancellation, $X''$ becomes exactly as in (\ref{revPoint}). Of course, for the sake of estimation, some $\sigma_0^2$ must be chosen. A numerically stable solution is to set $\sigma_0^2 = 2\max \{ \delta_j \}$.  
...
\label{projSD_algo}
\begin{algorithmic}[1]
\Require $\kappa \geq 0$ and sample eigenvalues in ascending order $\l_1 \leq l_2 \leq \dots \leq l_{N+1}$.
\Procedure{binary-search optimization}{}
\State Initialize $D \leftarrow \max\{l_1,0\}/\kappa$ and  $U \leftarrow l_{N+1}$.
...
\end{align}
First, it can be assumed that $\cond(h(\SS_Z)) = l_{N+1}/l_1 > \kappa$; otherwise, the projection can simply return $h(\SS_Z)$. Second, $l_1/\kappa \leq \mu \leq l_{N+1}$ because otherwise moving $\mu$ closer to the nearest sample eigenvalue decreases $g(\mu)$. Now, consider some value $\mu_n \geq 0$ and two index sets $\mathfrak{D}_n = \{i : l_i \leq \mu_n\}$ and $\mathfrak{U}_n = \{i : \mu_n\kappa \leq  l_i \}$. Then,
\begin{align*}
g(\mu_n) &= \sum_{i \in \mathfrak{D}_n}  \left(\mu-l_i\right)^2  + \sum_{i \in \mathfrak{U}_n} \left(l_i - \kappa\mu\right)^2,
\end{align*}
...
\end{align*}
 Of course, $\mu^*_n$ is the solution to (\ref{globalMu}) as long as moving from $\mu_n$ to $\mu^*_n$ does not change the index sets $\mathfrak{D}_n$ and $\mathfrak{U}_n$; that is, $\mu_n^* \in [\mathfrak{d}_n, \mathfrak{u}_n]$. If, on the other hand, $\mu_n^*$ is less than $\mathfrak{d}_n$ (or greater than $\mathfrak{u}_n$), the global minimum $\mu^*$ must be smaller than $\mathfrak{d}_n$ (or greater than $\mathfrak{u}_n$). If $\mu_n^*$ is, say, less than $\mathfrak{d}_n$, then a natural approach is to update $\mu_n$ to $\mu_{n+1}$ that is somewhere between $\mathfrak{d}_n$ and the smallest possible value of $\mu_{n+1}$.  This gives rise to a binary-search-like algorithm described in Algorithm \ref{projSD_algo}.
