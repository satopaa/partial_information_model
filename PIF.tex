...
\label{empirical}
This section evaluates a) the Gaussian model both on synthetic and real-world forecasting data, and b) illustrates how the Gaussian model can be applied to forecasting setups with different types of outcomes. Each case assumes $N$ forecasters attending $K$ prediction problems. The Gaussian model then requires the following components for problem $k$: the information variables $Z_{S,k}, Z_{B_1,k}, \dots, Z_{B_N,k}$, a target quantity $Y_k  \in \mathcal{Y}$, the forecasts $X_{jk} = \E[Y_k | Z_{B_j,k}]$, and a surjective link function $g: \R \to \mathcal{Y}$ such that $Y_k = g(Z_{S,k})$. 
%\end{enumerate}
%
%\begin{align*}
%%\text{Model Instance} = 
...
% For each event The  aggregators then observe $\Z_k^{-}$, where $\Z_k^{-} = (Z_{B_{1},k}, \dots, Z_{B_N, k})'$ holds only the forecasters' information variables such that $\Z_k = (Z_{S,k}, {\Z_k^{-}}')'$, 
 and predict $Y_{k}$ based on this information. The process is repeated a $2,000$ times under different values of $K$, ranging from $5$ to $35$ with constant increments of $5$. The number of forecasters $N$ is fixed at $20$. 
...
The revealed aggregator for event $k$ is $X_k'' = \diag(\bSigma_{22})' \bSigma_{22}^{-1}\X_k$. This, of course, requires an estimate of $\bSigma_{22}$. This subsection considers $X_k''$ under four different versions of $\bSigma_{22}$ in addition to two measurement error aggregators:
\begin{enumerate}[i)]
\item $X_k''$ with $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$, where $\kappa_{cov}$ is found over a grid of $10$ values between $10$ and $1,000$. 
...
Even though all estimators find $\bSigma_{22}$ more accurately as $K$ grows, $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ and $\SS_Z$ improve at a higher rate than $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$.  In fact, if $K > N$, $\SS_Z$ and $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ perform better than $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$. On the other hand, if $K < N$, $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$ is more accurate than the other two. Second, Figure \ref{SigmaInv}  presents the corresponding condition numbers. For the sake of keeping the scale manageable, $\cond(\SS_Z)$ has been omitted. Notice that $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{out}))$ increases while $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{cov}))$ generally decreases as $K$ grows larger. In fact, when $K > N$,  $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{cov}))$ is smaller than $\cond(\mathcal{P}_{LSE}(\cdot : \kappa_{out}))$. From the prediction perspective, this makes conditional-validation more forgiving towards error in the estimated $\bSigma_{22}$. Therefore, while $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$ incorporates the actual prediction process and looks for a fine balance between a precise estimate of $\bSigma_{22}$ and a low condition number, $\mathcal{P}_{LSE}(\cdot : \kappa_{out})$ is unaware of the details of revealed aggregation and hence simply focuses on estimating $\bSigma_{22}$ as accurately as possible. 
...
These two strategies lead to different predictive behavior as is illustrated in Figure \ref{ZsEstimation}. This plot shows the average squared error loss of the competing aggregators in predicting $Y_k$. Given that $Y_k = Z_{S,k} \sim \mathcal{N}(0,1)$, the loss of the prior mean $\E[(Y_k-0)^2] = 1.0$ can be considered as the upper bound in prediction error. The revealed aggregator $X_k''$ with the true information structure $\bSigma_{22}$, on the other hand, is optimal and hence provides the corresponding lower bound. Under most values of $K$, the revealed aggregator $X''$ with $\SS_Z$ receive a loss well above $1.0$ and is therefore not included in the figure. Overall, the average and median perform very similarly, with average loss fixed at around $0.26$. This is uniformly well above the loss of the revealed aggregator. The two versions of $X_k''$ perform very similarly when $K \geq N$. They monotonically tend close to the optimal at around $0.03$ as $K$ grows larger. However, when $K < N$, conditional validation is more robust and clearly yields better results. For this reason, only $\kappa_{cov}$ is considered in the following subsections that apply the Gaussian model to real-world forecasting data. 
...
\end{cases}
\end{align*}
where $t_k \in \R$ is some threshold value. That is, the link function $g(\cdot)$ is simply the indicator function $\one_{A_k}$ of the event $A_k = \{Z_{S,k} > t_k\}$. 
The thresholds are allowed to vary among events, hence permiting different prior distributions $\P(A_k)$. 
