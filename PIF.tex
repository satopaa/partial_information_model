...
% This section, however, focuses on predicting global events that are of particular interest to the Intelligence Advanced Research Projects Activity (IARPA).
 ...
 This paper develops methodology generally applicable for prediction polls. The focus is on empirical work and hence the methods are illustrated on real-world prediction polls. In particular, the first application is a forecasting tournament initiated in 2011 by the Intelligence Advanced Research Projects Activity (IARPA) as a means to acquire accurate predictions on important geopolitical events. Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
...
Once the predictions have been collected, the challenge is to combine them into a single consensus. This is typically done for the sake of decision-making and improved accuracy. Principled aggregation, however, requires an assumption about the source of heterogeneity among the forecasts. In particular, it is necessary to specify how the forecasts differ from the true value of the target quantity. For the past several decades, potentially due to the early forms of data collection, measurement error has served as the main source of heterogeneity. This approach has become the standard and is often applied in practice even when data variation is dominated  by causes besides error in measurement. For instance, the measurement error may be reasonable in modeling repeated estimates from a single instrument. However, it is unlikely to hold in prediction polling, where the estimates arise from multiple, often widely different sources. 
...
The main contribution of this paper is a new source of forecast heterogeneity, called \textit{information diversity}. 
This assumes variation to stem from information available to the forecasters and how they decide to use it. For instance, forecasters studying the same (or different) articles about a company may use separate parts of the information and hence report differing predictions on the company's future revenue. Such diversity forms the basis of a novel modeling framework known as the \textit{partial information framework}. This framework was originally introduced for probability forecasts in a theory-orientated paper by \cite{satopaamodeling}. The current paper, however, generalizes the framework, discusses model estimation, and derives specific partial information models for different types of forecasts, including point and probability estimates. Under each model the optimal aggregator is derived and evaluated on actual forecasting data. These aggregators outperform the current state-of-the-art aggregators arising from measurement error, hence providing practical evidence in favor of information diversity. 
...
The paper is structured as follows.  The next section reviews previous work on modeling forecasts. Section \ref{PIF} begins by describing the partial information framework at its most general level and then ends by introducing a close yet practical specification of the framework. This specification culminates in a distribution that describes information diversity fully through a covariance matrix. Section \ref{estimator} derives a numerical procedure that efficiently estimates the information structure among the forecasters. Section \ref{empirical} illustrates the framework with applications to synthetic and real-world forecasts of different types of outcomes. In particular, the partial information framework is used to analyze probability forecasts from the second year of the GJP and point estimates from... The final section concludes with a summary and discussion of
future research.
