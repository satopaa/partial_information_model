...
The current paper introduces statistical methodology designed specifically for the rapidly growing practice of prediction polling. The methods are illustrated on real-world data involving two common types of responses, namely prediction and point forecasts. In particular, the probability forecasts are from a tournament initiated in 2011 by the Intelligence Advanced Research Projects Activity (IARPA) as means to acquire accurate predictions on future international political events. Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
...
the \textit{information set} used by the forecaster. In particular, if $\F_j = \sigma(X_j)$ is the $\sigma$-field generated by $X_j$, then $X_j = \E[Y \| \F_j]$ is nothing but the property of conditional unbiasedness discussed in the previous subsection. It is important to note that Forecaster $j$ may have access to some larger $\sigma$-field $\G$ but only decides to use $\F_j \subseteq \G$. For the purposes of aggregation, however, any available information discarded by the forecaster may as well not exist. 
All the forecasters operate under the same probability model but use different information sets in their predictions.
