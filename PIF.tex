...
Second, given that the current paper does not focus on modeling dynamic data, only forecasts made within some time interval are considered.  The final outcome is typically the most uncertain at the beginning and then becomes more certain towards the resolution date. The goal is to choose some mid-interval during which predicting the correct outcome is neither trivial nor extremely difficult because in such cases the competing aggregators are unlikely to present meaningful differences. 
...
However, evaluating different techniques to handle missing values is well outside the scope of this paper. Therefore only the hundred most active forecasters are considered. 

All these considerations led to a parallel analysis of two scenarios: 
 \begin{enumerate}[a)]
...
\item \textit{Higher Uncertainty (HU):} Consider each forecaster's most recent forecast during  $90-120$ days before the anticipated resolution day. The resulting dataset has $49$ events of which $10$ occurred. There are $732$ forecasters attending, on average, $7.7$ problems, with a range of $1$ to $46$ problems.  The top $100$ most active forecasters attended, on average, $24.0$ problems, with a range of $14$ to $46$ problems. 
 \end{enumerate}
Another study looking at the $60-90$ days before the anticipated resolution day was also conducted. The results, however, were qualitatively very similar to the results currently presented. Therefore, to avoid redundancy, this middle scenario was omitted from the paper.  
...
%\noindent
%\textit{Model.} 
The target events considered in this section have two possible outcomes; that is, $Y_k \in \{0,1\}$ for all $k = 1, \dots, K$. The outcomes are linked to the Gaussian information pool via the following function:
\begin{align*}
...
Note that the thresholds are allowed to vary the among events, hence permitting different prior distributions $\P(A_k)$. 
...
%\noindent
%\textit{Analysis.} 
The accuracy of the reveled aggregator $X_k''$ is compared against the state-of-the-art measurement error aggregators, namely the average probability, median probability, average probit-score, and average log-odds. To avoid infinite log-odds
and probit scores, extreme forecasts $X_{jk} = 0$ and $1$ were
censored to $X_{jk} = 0.001$ and $0.999$, respectively. The accuracy of an aggregate $X_k$ is measured with the mean Brier score (BS) defined as
%Consider $K$
...
Given that $X_k''$ presented the highest relative improvement under LU, only this scenario is analyzed. 
