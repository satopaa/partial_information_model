...
The current paper introduces statistical methodology designed specifically for the rapidly growing practice of prediction polling. The focus, however, is on empirical work. Therefore, the methods are illustrated with case studies of real-world prediction polls.  In particular, the first study analyzes a forecasting tournament initiated in 2011 by the Intelligence Advanced Research Projects Activity (IARPA) as means to acquire accurate predictions on future international political events. Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
...
These forecasters were then asked to estimate probabilities of the events specified by IARPA.  To illustrate, Figure \ref{Examples} shows the forecasts for two of these events.
...
In general, the forecasters were allowed to make and update their estimates until the events resolved. The forecasters, however, reported updates very infrequently. Furthermore, not all forecasters made probability estimates for all the events, making the dataset extremely sparse. 
...
example, consider two forecasters who visit a company and predict its future revenue. One forecaster may carefully examine the company's technological status while while the other pays closer attention to what the managers say.  Even though the
...
This framework has become standard practice possibly because of its simplicity and familiarity.  Unfortunately, there are a number of disadvantages. First, average-based aggregators target the mean of forecasts. This, however, may not be the correct target. For instance, the correct target in probability forecasting is the event indicator that lies on the boundary of
the sample space. 
...
\begin{enumerate}[i)]
\item $X_k''$ with $\mathcal{P}_{LSE}(\cdot : \kappa_{cov})$, where $\kappa_{cov}$ is found over a grid of $100$ values between $10$ and $1,000$. 
