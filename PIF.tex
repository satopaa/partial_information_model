...
\subsection{Noise, Bias, and Assessment}
\label{context}



Consider $N$ forecaster and suppose Forecaster $j$ predicts $X_j$ for some (random) quantity of interest $Y$. The prediction $X_j$ is simply an estimator of $Y$. Therefore, as is the case with all estimators, 
...
A forecast is typically assessed with a loss
function $L(X_i, Y)$. Many choices, however, are possible, and
...
Forecaster~$j$ forms an opinion based on $L_j (H_1 , \ldots , H_r)$,
where each $L_j$ is a linear function of cues
