...
$j$. This term is not necessarily positive because being positively correlated is not transitive. More precisely, by \citet[Theorem 3]{langford2001property}, $\Cov (X_{i} , X_{j}) \geq \Var(X_j)\Var(X_i) - \sqrt{\Var(X_j)(1-\Var(X_j))\Var(X_i)(1-\Var(X_i))}$, which shows that $\Cov (X_{i} , X_{j}) \geq 0$ if $ \Var(X_j) + \Var(X_i) \geq 1$. 
...
% \end{array}\right)\right).  \label{NExperts}
%\end{align}
The target quantity is then given by $Y = g(Z_Y)$, where $g$ is a context-specific link function.  In general it makes sense to have $g$ map the real numbers to the support of $Y$. For instance, if $Y \in \{0,1\}$, then $g : \R \to \{0,1\}$. The fact that $\Var(Z_Y) = 1$ is irrelevant as this choice can be compensated by the link function.
