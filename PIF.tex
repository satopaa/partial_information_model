...
$j$. Therefore the sub-matrix $\bSigma_{22}$ is naturally called the \textit{information structure}. The information variables link to $Y$ and the $X_j$'s via a \textit{link function} and the model assumptions. First, if $\mathcal{Y}$ denotes the support of $Y$, then 
 $Y = g(Z_0)$, where $g: \R \to \mathcal{Y}$ is some surjective link function specific to the model instance. For instance, if the prior distribution of $Y$ is $\mathcal{N}(\mu_0, \sigma^2_0)$, then $\mathcal{Y} = \R$ and $g(Z_0) = Z_0 \sigma_0 + \mu_0$.  Therefore  the link function is assumed to transform $Z_0$ such that $Y$ follows the correct prior distribution. Second, given that the forecasts are conditionally unbiased, Forecaster $j$ predicts $X_j = \E[Y | Z_j]$. This gives a very flexible specification, yielding a broad range  of partial information models. Examples are discussed in Section \ref{empirical} that illustrates this \textit{Gaussian model} on several different types of outcomes.  
...
The choice of the Gaussian distribution can be motivated both from the intuitive and technical points of view. First, \citet{broomell2009experts} discuss a model for subjective interpretation. They assume that
...
to model the forecasters' observations as jointly Gaussian.  Second, both the multivariate and conditional Gaussian distributions are well-studied and have simple forms. This is especially useful in deriving the revealed aggregator.   In particular, if $\Z = (Z_{1}, \dots, Z_{N})'$, then $$Z_0 | \Z \sim \mathcal{N}\left(\bSigma_{12} \bSigma_{22}^{-1} \Z, 1-\bSigma_{12} \bSigma_{22}^{-1}\bSigma_{21}\right).$$ This leads to a closed-form revealed aggregator $X''$ in many different applications of the model. For more motivation and interpretation, see \cite{satopaamodeling}.
...
\begin{quote}
{\bf Model Instance.}  The link function $g$ is the identity, the target quantity is $Y_k = g(Z_{0k}) = Z_{0k}$, and the forecasts are $X_{jk} = \E[Y_k | Z_{jk}] = Z_{jk}$ for $j = 1, \dots, N$ and $k = 1, \dots, K$. The revealed aggregator for problem $k$ is $X_k'' = \bSigma_{12} \bSigma_{22}^{-1} \X_k$, where $\X_k = \Z_k$. 
\end{quote}


Simulating forecasts requires a covariance matrix $\bSigma_{22}$ such that $h(\bSigma_{22}) \in \mathcal{S}_{+}^{N+1}$. A mechanism for randomly generating valid information structures is introduced in the following proposition. 
\begin{proposition}
\label{synth}
...
\boldsymbol{x}\boldsymbol{x}' : \boldsymbol{x} \in
\{0,1\}^N\right\}$. Begin by approximating the full information with a finite set of $L$ elements. Describe the $j$th forecaster's information with a binary vector $\v_j$ whose $L$ entries indicate which elements of the full information the forecaster knows. Suppose this vector is constructed by making $L$ independent draws from a Bernoulli distribution with success probability $\delta_j \in [0,1]$. If $\V$ is a $N \times L$ matrix with rows equal to the forecasters' information vectors, the corresponding (random) information structure is $\V \V' / L$, which, by construction, is always in $\COR_N$. Given that $\COR_N$ is a convex set, the expectation of  $\V \V' / L$ is also in $\COR_N$. The diagonal and off-diagonal elements of this expectation are the desired $\E[\v_j' \v_j] = \delta_j$ and $\E[\v_i' \v_j] = \delta_i \delta_j$, respectively.  Finally, \cite{laurent1997connections} showed that all $\bSigma_{22} \in \COR_N$ satisfy $h(\bSigma_{22}) \in \mathcal{S}_{+}^{N+1}$. 
\end{proof}
