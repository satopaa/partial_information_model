...
% $X_j = \E [Y | \F_j]$ only requires the existence of a probability model, together with the assumption of conditional unbiasedness. 
 ...
 \begin{proposition}
\label{covstr}
If $\E(Y|\F_j)  = X_j$ for some $\F_j \subseteq \F$ and all $j =1, \dots, N$, then the following holds for all $i$ and $j$ .
\begin{enumerate}[i)] 
...
to model the forecasters' observations as jointly Gaussian. This distributional choice has been utilized by other authors in the past (see, e.g., \citealt{broomell2009experts, Bacco}). 
