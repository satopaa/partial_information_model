...
The current paper introduces statistical methodology designed specifically for the rapidly growing practice of prediction polling. The focus, however, is on empirical work. Therefore, the methods are illustrated with case studies of real-world prediction polls.  The first study analyzes a forecasting tournament initiated in 2011 by the Intelligence Advanced Research Projects Activity (IARPA) as means to acquire accurate predictions on future international political events. Among the participating teams, the Good Judgment Project (GJP) (\citealt{ungar2012good, mellers2014psychological}) has emerged as the clear winner. 
...
These elements, however, differ largely in their sensitivity to numerical operations. This is important because an accurate yet highly sensitive estimate of $\bSigma_{22}$ can lead to imprecise aggregation. To make this concrete, recall from Section \ref{gaussian} that the conditional mean of $Z_S$ given $\Z$ is  $\bSigma_{12} \bSigma_{22}^{-1} \Z$. Given that in many applications this term ends up propagating to the final revealed aggregator $X''$, it deserves careful estimation. 
%notice that the Gaussian revealed aggregator includes the term $\bSigma_{12} \bSigma_{22}^{-1} \Z$.
 Re-express the term as $\w' \Z$, where $\w$ is the solution to $\bSigma_{12} = \w' \bSigma_{22}$. The rate at which the solution changes with respect to a change in $\bSigma_{12}$ depends on the condition number $\cond(\bSigma_{22}) := \lambda_{max}(\bSigma_{22}) / \lambda_{min}(\bSigma_{22})$, i.e., the ratio between the maximum and the minimum eigenvalues of $\bSigma_{22}$. If the condition number is very large, a small error in $\bSigma_{12}$ can cause a large error in $\w$. In the contrary, if the condition number is small, $\bSigma_{22}$ is \textit{well-conditioned} and error in $\w$ will not be much larger than the error in $\bSigma_{12}$. Therefore, to prevent error from being largely amplified during aggregation, the estimation procedure includes an additional constraint:
  \begin{align}
 \cond(\bSigma_{22}) \leq \kappa, \label{condNum}
 \end{align}
where $\kappa \geq 1$ is a given threshold. 
...
Both constraints (\ref{CORn}) and (\ref{condNum}) form convex sets. The corresponding intersection is the feasible region of the general estimation problem. To make this specific, note that the sample covariance matrix $\boldsymbol{S}_Z$ of $\Z$ is sufficient for $\bSigma_{22}$. Therefore the general estimation problem can be written as
\begin{align}
\label{orig_problem}
\begin{split}
...
In many applications $\bSigma_{22}$ can be estimated easily with an unconstrained estimator such as the sample covariance matrix $\SS_Z$. Even though this section focuses on $\SS_Z$, the discussion generalizes to any unconstrained estimator via direct substitution of $\SS_Z$. Any such estimator can be improved by projecting it onto the feasible region. This approach, known sometimes as least squares estimation (LSE), has been studied extensively by many authors  (see, e.g., \citealt{soloveychik2013covariance, barton1997structured, henrion2012projection, malick2004dual, hu1991numerical}). It guarantees a globally optimal solution and facilitates  dimension reduction. 
...
Both optimization problems (\ref{kappa_out}) and (\ref{kappa_cov}) are non-convex in $\nu$.  However, as was mentioned before, Algorithm \ref{proj_algo} is fast for moderately sized $N$. Therefore $\kappa$ can be chosen efficiently over a  grid of candidate values.  In practice an extremely accurate selection of $\kappa$ is hardly necessary. Therefore, especially when $N$ is large, computation time can be saved largely by choosing $\kappa$ with a cruder tolerance.  For instance, in the following empirical section, $\kappa$ is chosen over a grid of $100$ values between $10$ and $1,000$. 
...
% For each event The  aggregators then observe $\Z_k^{-}$, where $\Z_k^{-} = (Z_{B_{1},k}, \dots, Z_{B_N, k})'$ holds only the forecasters' information variables such that $\Z_k = (Z_{S,k}, {\Z_k^{-}}')'$, 
 and predict $Y_{k}$ based on this information. The process is repeated a $5,000$ times under different values of $K$, ranging from $5$ to $35$ with constant increments of $5$. The number of forecasters $N$ is fixed at $20$. 
...
This subsection compares the aggregation accuracy of  two measurement error aggregators and the revealed aggregator $X_k''$ under four different versions of $\bSigma_{22}$. More specifically, the competing methods are:
\begin{enumerate}[i)]
...
\item The revealed aggregator $X_k''$ with $\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov})$. Call this aggregator $X_{cov}''$. 
...
The cross-validation uses five folds; that is,  $R = 5$. Call this aggregator $X_{out}''$. 
\item  The revealed aggregator  $X_k''$ with the true information structure $\bSigma_{22}$. Call this aggregator $X_{true}''$. 
...
Even though all estimators find $\bSigma_{22}$ more accurately as $K$ grows, $\mathcal{P}_{LSE}(\SS_Z : \kappa_{out})$ and $\SS_Z$ improve at a higher rate than $\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov})$.  In fact, if $K > N$, $\SS_Z$ and $\mathcal{P}_{LSE}(\SS_Z : \kappa_{out})$ perform better than $\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov})$. On the other hand, if $K < N$, $\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov})$ is more accurate than the other two. Second, Figure \ref{SigmaInv}  presents the corresponding (average) condition numbers. For the sake of keeping the scale manageable, $\cond(\SS_Z)$ has been omitted. Notice that $\cond(\mathcal{P}_{LSE}(\SS_Z : \kappa_{out}))$ increases while $\cond(\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov}))$ generally decreases as $K$ grows larger. In fact, when $K > N$,  $\cond(\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov}))$ is smaller than $\cond(\mathcal{P}_{LSE}(\SS_Z : \kappa_{out}))$. From the prediction perspective, this makes conditional-validation more forgiving towards error in the estimated $\bSigma_{22}$. Therefore, while $\mathcal{P}_{LSE}(\SS_Z : \kappa_{cov})$ incorporates the actual prediction process and looks for a fine balance between a precise estimate of $\bSigma_{22}$ and a low condition number, $\mathcal{P}_{LSE}(\SS_Z : \kappa_{out})$ is unaware of the details of revealed aggregation and hence simply focuses on estimating $\bSigma_{22}$ as accurately as possible. 
...
These two strategies lead to different predictive behavior as is illustrated in Figure \ref{ZsEstimation}. This plot shows the average squared error loss of the competing aggregators in predicting $Y_k$. Given that $Y_k = Z_{S,k} \sim \mathcal{N}(0,1)$, the loss of the prior mean $\E[(Y_k-0)^2] = 1.0$ can be considered as the upper bound in prediction error. The revealed aggregator $X_{true}''$, on the other hand, is optimal and hence provides the corresponding lower bound. Under most values of $K$, the revealed aggregator $X''$ with $\SS_Z$ receive a loss much larger than $1.0$ and is therefore not included in the figure. Overall, the average and median perform very similarly, with average loss fixed at around $0.26$. This is uniformly well above the loss of the revealed aggregator, suggesting that averaging aggregators are a poor choice when forecasts truly arise from a partial information model. The two revealed aggregators $X_{cov}''$ and $X_{out}''$ perform very similarly when $K \geq N$. They monotonically tend close to the optimal at around $0.03$ as $K$ grows larger. However, when $K < N$, conditional validation is more robust and clearly yields better results. For this reason, only $X_{cov}''$ is considered in the following subsections that apply the Gaussian model to real-world forecasting data. 
...
Second, given that the current paper does not focus on modeling dynamic data, only forecasts made within some time interval are considered.  Note that the final outcome is typically the most uncertain in the beginning and then becomes fully certain by the resolution date. The goal therefore is to choose some mid-interval during which predicting the correct outcome is neither trivial nor extremely difficult because in both of these cases the competing aggregators are unlikely to present meaningful differences. 
...
Evaluating different techniques to handle missing values, however, is well outside the scope of this paper. Therefore only the hundred most active forecasters are considered. This makes sufficient overlap highly likely but still not guaranteed. 

All these considerations lead to a parallel analysis of two scenarios: 
 \begin{enumerate}[a)]
...
\end{cases}
\end{align*}
where $t_k \in \R$ is some threshold value. Therefore the link function $g(\cdot)$ is simply the indicator function $\one_{A_k}$ of the event $A_k = \{Z_{S,k} > t_k\}$. The prior probability of the $k$th event is $\P(Y_k = 1) = \Phi(-t_k)$, where $\Phi(\cdot)$ is the CDF of a standard Gaussian distribution. Given that the thresholds are allowed to vary among the events, each event has its own prior. 
...
\end{align*}
where $\hat{\bSigma}_{22} = \mathcal{P}_{LSE}(\SS_Z : \nu)$. This procedure is repeated over a grid of $100$ candidate values $\nu$ between $10$ and $1,000$. The final value is chosen with the conditional validation technique discussed in Section \ref{condSelection}; that is, the resulting revealed aggregator is $X_{cov}''$. 
...
%\noindent
%\textit{Analysis.} 
This aggregator benchmarked against the state-of-the-art measurement error aggregators, namely the average probability, median probability, average probit-score, and average log-odds. To avoid infinite log-odds
...
Figure \ref{simReal} shows the average BS under HU and LU. Notice that the scores in Figure \ref{highUnc} are uniformly worse than in Figure \ref{lowUnc}. This reflects the higher level of uncertainty in HU. Both scenarios, however, tell a similar story about the relative accuracies of the competing aggregators. First, the measurement error aggregators rank in the typical order (from worst to best): average probability, median probability, average probit, and average log-odds. Second, independently of $N$ and the level of uncertainty, the revealed aggregator $X_{cov}''$ outperforms the averaging aggregators by a noticeable margin. Third,  $X_{cov}''$ is able to collect information more efficiently than the measurement error aggregators. Consequently, its performance advantage grows as $N$ increases. This suggests that, at least in the GJP data, information diversity is a better model of forecast heterogeneity than measurement error. 
