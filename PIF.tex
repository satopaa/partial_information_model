...
\label{estimator}
This section describes methodology for estimating the information structure $\bSigma_{22}$.  The predictions provide signal on $\bSigma_{22}$ as follows: a) the more similar two forecasters' predictions are, the higher the corresponding information overlap; and b) the more unlikely a forecast is according to the non-informative prior, i.e., the belief only reflecting the basic information, the more informed the corresponding forecaster is likely to be. 
...
These elements, however, differ largely in their sensitivity to numerical operations. This is important because an accurate yet highly sensitive estimate of $\bSigma_{12}$ can lead to imprecise aggregation. To make this concrete, recall from Section \ref{gaussian} that the conditional mean of $Z_S$ given $\Z$ is  $\bSigma_{12} \bSigma_{22}^{-1} \Z$. Given that in many applications this term ends up propagating to the final revealed aggregator $X''$, it deserves careful estimation. 
%notice that the Gaussian revealed aggregator includes the term $\bSigma_{12} \bSigma_{22}^{-1} \Z$.
 Re-express the term as $\w' \Z$, where $\w$ is the solution to $\bSigma_{12} = \w' \bSigma_{22}$. The rate at which the solution changes with respect to a change in $\bSigma_{12}$ depends on the condition number $\cond(\bSigma_{22}) := \lambda_{max}(\bSigma_{22}) / \lambda_{min}(\bSigma_{22})$, i.e., the ratio between the maximum and the minimum eigenvalues of $\bSigma_{22}$. If the condition number is very large, a small error in $\bSigma_{12}$ can cause a large error in $\w$. In the contrary, if the condition number is small, $\bSigma_{22}$ is \textit{well-conditioned} and error in $\w$ will not be much larger than the error in $\bSigma_{12}$. Therefore the estimation procedure should require
  \begin{align}
 \cond(\bSigma_{22}) \leq \kappa \label{condNum}
 \end{align}
 for a given threshold $\kappa \geq 1$.
...
Both constraints (\ref{CORn}) and (\ref{condNum}) form convex sets. The corresponding intersection is the feasible region of the general estimation problem. To make this specific, let $\boldsymbol{S}_Z$ denote the sample covariance matrix of $\Z$. Under the Gaussian model this is sufficient for $\bSigma_{22}$. Therefore the general estimation procedure is
\begin{align}
\label{orig_problem}
\begin{split}
...
Typically the global optimum to such a problem can be found very efficiently. There are, however, two difficulties: 
\begin{enumerate}[(i)]
...
\end{align*}
where $\diag(\M)$ is a vector of the diagonal elements of $\M$.  The correlation polytope is then relaxed with $\SD_N := \left\{ \M \in \mathcal{S}^N : h(\M) \in \mathcal{S}_{+}^{N+1} \right\}$. Given that $\bSigma := h(\bSigma_{22})$ is the covariance matrix of the multivariate Gaussian distribution (\ref{NExperts}), having $h(\bSigma_{22}) \in \SD_N$ can be viewed as the minimal requirement for $\bSigma_{22}$. This condition was discussed by \cite{laurent1997connections}, who showed that $\COR_N \subseteq \SD_N$ and called it, ``in some sense, the best possible relaxation of'' $\COR_N$. An equivalent relaxation has been extensively studied in the context of the well-known Max-Cut problem, where it has been found to be surprisingly strong both in theory and practice (see, e.g., \citealt{wolkowicz2000handbook}). 
...
The true information structure $\bSigma_{22}$ can be often estimated easily with an unconstrained estimator such as the sample covariance matrix $\SS_Z$. Even though this section focuses on $\SS_Z$, the discussion generalizes to any unconstrained estimator via direct substitution of $\SS_Z$. Any such estimator can be improved by projecting it onto the feasible region. This approach, known sometimes as least squares estimation (LSE), has been studied extensively by many authors  (see, e.g., \citealt{soloveychik2013covariance, barton1997structured, henrion2012projection, malick2004dual, hu1991numerical}). It guarantees a globally optimal solution and facilitates  dimension reduction. 
...
The first step is to make a change of variables and project $h(\boldsymbol{S}_Z)$ instead of $\boldsymbol{S}_Z$. 
...
%This, however, only affects the rate at which region grows in $\kappa$. 
 ...
 Section \ref{condSelection} introduces a procedure to choose $\kappa$ from the data such that the final estimate of $\bSigma_{22}$ is well-conditioned. Under such an adaptive procedure, problem (\ref{lse_problem}) can be considered equivalent to the  relaxed problem (\ref{relax_problem}) (with the objective $f_0$ equal to the Frobenius norm).
...
If $\A$ is a matrix with the $j$th row equal to $\vec(\A_j)$ for $j = 1, \dots, N+1$, then the  linear constraints in (\ref{relax_problem}) can be expressed as $\A\m =  \boldsymbol{e}_1$, where $\boldsymbol{e}_1$ is the first standard basis vector. The projection of  $\M$ onto $\mathcal{C}_{lin}$ is $\vec^{-1}(\m + \A' (\A \A')^{-1}(\boldsymbol{e}_1 - \A\m))$. This expression simplifies significantly by close inspection. In particular, the projection is equivalent to setting $m_{11} = 1$ and replacing $m_{j1}$, $m_{1j}$, and $m_{jj}$ by their average $(m_{jj} + m_{j1} + m_{1j})/3$, for $j = 2, \dots, N+1$. Denote this projection with the operator $\mathcal{P}_{lin}(\cdot)$. 

Second, \cite{tanaka2014positive} re-express the projection of $\M \in \mathcal{S}^{N+1}$ onto $\mathcal{C}_{sd}$ as a univariate optimization problem. To  describe this approach, consider the spectral decomposition $\M = \Q \Diag(l_1, \dots, l_{N+1}) \Q'$ and the univariate function 
\begin{align*}
g(\mu) &= \sum_{i=1}^{N+1} \left[ \left(\mu-l_i\right)^2_+ + \left(l_i - \kappa\mu\right)^2_+ \right],
\end{align*}
where $\Diag(\x)$ is a diagonal matrix with diagonal $\x$ and $(\cdot)_+$ is the positive part operator. The function $g(\mu)$ can be minimized very efficiently by solving a series of smaller convex problems, each with a closed form solution. Complete technical details of this are given in the Appendix.
...
Given that Algorithm \ref{proj_algo} requires on a given $\kappa$, a principled approach to selecting this threshold should be described. 
...
\marginpar{Change the caption to hold the actual sample cov matrices. Bold things.}
...
\end{enumerate}
\marginpar{Call revealed aggs as $X_{cov}''$}
