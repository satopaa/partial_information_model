...
%More specifically,
forecaster $j$ predicts $X_j = \E(Y \| \F_j)$ based on some partial \textit{information set} $\F_j \subseteq \F$. Therefore $\F_i \neq \F_j$ if $X_i \neq X_j$, and forecast heterogeneity stems purely from \textit{information diversity}. This point of view was motivated in \cite{satopaamodeling} via simple examples illustrating that the optimal aggregate is not well-defined without assumptions on the information structure among the forecasters. Note, however, that if forecaster $j$ uses a simple rule, $\F_i$ may not be the full $\sigma$-field of information available to the forecaster but rather a smaller $\sigma$-field corresponding to the information used by the rule. Furthermore, if two forecasters have access to the same $\sigma$-field, they may decide to use different sub-$\sigma$-fields, leading to different predictions. 
...
% $X_j = \E [Y | \F_j]$ only requires the existence of a probability model, together with the assumption of conditional unbiasedness. 
 ...
 \begin{proposition}
\label{covstr}
If $\E(Y|\F_j)  = X_j$ for some $\F_j \subseteq \F$ and all $j =1, \dots, N$, then the following holds.
\begin{enumerate}[i)] 
\item The forecasts are marginally consistent: $\E(Y) = \E(X_j)$ for all $j = 1, \dots, N$. \label{first}
\item $\Cov(X_j, X_i) = \Var(X_i)$ for all $i$ such that $\F_i \subseteq \F_j$.
   \label{second}
\item $\Var(X_i) \leq \Var(X_j)$ for all $i$ such that $\F_i \subseteq \F_j$.  \label{third}
\end{enumerate}
\end{proposition}
...
$j$. Observe that under the partial information framework increased variance is generally deemed helpful. Contrast this with the standard statistical models that often regard higher variance as increased noise. 
...
to model the forecasters' observations as jointly Gaussian. This distributional choice has been utilized by many authors in the past (see, e.g., \citealt{broomell2009experts, Bacco}). 
...
Unfortunately, in many applications $Y$ and $X_j$ may not be real-valued. For instance, in probability forecasting of binary events typically  $Y \in \{0,1\}$ and $X_j \in [0,1]$. This limitation, however, can be handled by borrowing from the theory of generalized linear models \citep{mccullagh1989generalized} and utilizing a \textit{link function}. The following \textit{Gaussian model} makes this concrete by first introducing $N+1$ auxiliary variables, called the \textit{information variables}, that follow a multivariate Gaussian distribution with the covariance pattern (\ref{cov_str}):
\begin{align}
...
% \end{array}\right)\right).  \label{NExperts}
%\end{align}
The target outcome is then given by $Y = g(Z_Y)$, where $g(\cdot)$ is an application-specific link function mapping from the real numbers to the support of $Y$. For instance, if $Y \in \{0,1\}$, then $g : \R \to \{0,1\}$. The fact that $\Var(Z_Y) = 1$ is irrelevant because this choice can be compensated by the link function.
...
% $Y = g(Z_Y)$ for some surjective \textit{link function} $g: \R \to \mathcal{Y}$ specified by the model instance. For instance, if $Y$ is continuous with a prior distribution $\mathcal{N}(\mu_0, \sigma^2_0)$, then $\mathcal{Y} = \R$ and $g(Z_Y) = Z_Y \sigma_Y + \mu_0$. 
 ...
 The remaining information variables, namely $Z_j$ for $j =1, \dots, N$ summarize the forecasters' observed information. In particular, forecaster $j$ makes a prediction about $Y$ conditional on observing $Z_j$. The revealed aggregator, on the other hand, estimates $Y$ conditional on all the $Z_j$'s. Therefore $X_j = \E(Y | Z_j)$ for all $j = 1, \dots, N$ and $X'' = \E(Y | Z_1, \dots, Z_N)$.  These expressions can be often derived by first recalling the form of the Gaussian conditional distributions:
\begin{align*}
Z_{Y|Z_j} &:= Z_Y | Z_j \sim \mathcal{N}\left(Z_j, 1-\delta_j\right) \text{ and }\\
Z_{Y|\Z} &:= Z_Y | \Z \sim \mathcal{N}\left(\diag(\bSigma)' \bSigma^{-1} \Z, 1-\diag(\bSigma)' \bSigma^{-1}\diag(\bSigma)\right),
\end{align*}
where $\Z = (Z_1, \dots, Z_N)'$. The individual forecasts and the revealed aggregator then are $\E(Y | Z_j) = \E\{g(Z_{Y|Z_j})\}$ and $\E(Y | \Z) = \E\{g(Z_{Y|\Z})\}$, respectively. 

Given that these expressions and the construction of $Y$ only depend on the choice of $g(\cdot)$, each instance of the Gaussian model is fully specified by its link function. One of the most important instances addresses point forecasts $X_j$ of a continuous $Y$. The next proposition considers this instance and analyzes the relationship between $X''$ and the weighted average of the forecasts. 

\begin{proposition} \label{Gaussian}
Consider the Gaussian model for point forecasts of continuous outcomes. Denote the prior on the outcome with $Y \sim \mathcal{N}(\mu_Y, \sigma_Y^2)$. If $\X = (X_1, \dots, X_N)'$, then the revealed aggregator is $X'' = \diag(\bSigma)'\bSigma^{-1}(\X-\mu_Y\one_N) + \mu_Y$.  Consider the weighted average $\w'\X$, where $w_1, \dots, w_N \geq 0$ and $\sum_{i=1}^N w_j = 1$. Then,
%\begin{enumerate}
...
%\end{enumerate}
\end{proposition}
\marginpar{Be careful here though. Make a similar claim to the reviewers comment. The aggregator only cares about the total noise in that region etc.}
Recall from Section \ref{context} that $X''$ minimizes the expected quadratic loss among all functions that are measurable with respect to $\F''$. Given that $\w' \X \in \F''$, no weighted average can outperform $X''$ in terms of expected quadratic loss. In particular, $\w' \X$ for any set of weights $\w$ is strictly sub-optimal except in the extreme case when one forecaster knows everything that all the other forecasters know. This result is illustrated under synthetic data in Section \ref{simulation} and verfiied under real-world data in Section \ref{continuousReal}.
